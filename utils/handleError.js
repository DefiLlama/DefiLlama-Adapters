...
function handleError(error) {
  console.log("\n", errorString, "\n\n");
  const isGraphError =
    error.stack?.includes("graphql-request") && error.response?.errors?.length;
  if (isGraphError)
    console.error(error.response.errors.map((e) => e.message).join("\n"));
  else console.error(error.toString());
  const axiosError = error?.response?.data?.message;
  if (axiosError) console.log("Axios: ", axiosError);
...
function getStackMessage(stack) {
  ...
  const message = [];
  stack = stack.split("\n");
  while (stack.length && !stack[0].includes("  at ")) stack.shift();
  const firstNMStackMessage = stack
    .filter(isNodeMolule)
