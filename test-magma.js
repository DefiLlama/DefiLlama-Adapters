...
(async () => {
  ...
  const { chainBlocks } = await getCurrentBlocks(["iotex"]); // fetch only ethereum block for local test
  const ethBlock = chainBlocks["iotex"];
  const usdTvls = {};
...
function fixBalances(balances) {
  ...
  Object.entries(balances).forEach(([token, value]) => {
    let newKey
    if (token.startsWith("0x")) newKey = `${globalChain}:${token}`
...
Warning: `)
  ...
  // console.table(usdTokenBalances)
