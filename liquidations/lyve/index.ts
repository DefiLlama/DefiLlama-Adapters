...
const ADMIN_CONTRACT_ADDRESS = "0xcFcd25b5E200d8829c383d293B456a608777a1D8";
const VESSEL_MANAGER_CONTRACT_ADDRESS = "0xD742abB760AAc756A7253CC2D58C80450C729b63";
const BLYU_CONTRACT_ADDRESS = "0x0000000000000000000000000000000000000000";
const WETH_CONTRACT_ADDRESS = "0xe5D7C2a44FfDDf6b295A15c148167daaAf5Cf34f";

const EXPLORER_BASE_URL = "https://lineascan.build/address/";
...
const tokenToCollateralAddress = (token: string) => {
  if (token === WETH_CONTRACT_ADDRESS) {
    return "linea:0x0000000000000000000000000000000000000000"
  }
  return "linea:" + token
}
...
const positions = async () => {
  ...
  collAddresses = collAddresses.filter(i => i !== BLYU_CONTRACT_ADDRESS) // BLYU is not liquidatable

  // get prices for calculating recovery mode
  const prices = {}
  for (const c of collAddresses) {
    const info = await getTokenInfo(`linea:${c}`)
