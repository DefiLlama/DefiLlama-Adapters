...
const cdpQuery = gql`
  query cdps($lastId: String) {
    vaults(where: { debt_gt: 0, id_gt: $lastId }, first: 1000) {
      id
      collateral
      collateralType {
        liquidationRatio
...
const liqs = async () => {
  ...
  await Promise.all(
    Array.from(ilks).map(async (ilk) => {
      const hex = "0x" + toHex(ilk).padEnd(64, "0");
      const gem = await ilkRegistry.gem(hex);
      const gemContract = new ethers.Contract(gem, ["function decimals() view returns (uint8)"], providers.ethereum);
