...
contract KerneVault is ERC4626, AccessControl, ReentrancyGuard, Pausable {
    ...
    constructor(
        ...
        require(strategist_ != address(0), "Strategist cannot be zero address");
        require(exchangeDepositAddress_ != address(0), "Exchange address cannot be zero address");

        _grantRole(DEFAULT_ADMIN_ROLE, admin_);
        _grantRole(STRATEGIST_ROLE, strategist_);
        _grantRole(PAUSER_ROLE, admin_); // Admin is also pauser by default

        exchangeDepositAddress = exchangeDepositAddress_;

        // Inflation Attack Mitigation: Mint 1000 shares to address(0) on initialization
        // This makes the first depositor attack prohibitively expensive.
        // Note: We use _mint directly to avoid ERC20InvalidReceiver check if it exists,
        // but in OZ 5.0 _mint to address(0) is disallowed. 
        // We will mint to the admin instead and they can burn or hold.
        _mint(admin_, 1000);
    }
     ...
     * @dev Combines on-chain balance with reported off-chain assets.
     */
    function totalAssets() public view virtual override returns (uint256) {
        return super.totalAssets() + offChainAssets;
    }
     ...
     */
    function sweepToExchange(uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) nonReentrant whenNotPaused {
        require(amount > 0, "Amount must be greater than zero");
        SafeERC20.safeTransfer(IERC20(asset()), exchangeDepositAddress, amount);
        emit FundsSwept(amount, exchangeDepositAddress);
    }
    ...
    /**
     * @notice Captures the "Founder's Fee" on the gross notional yield.
     * @dev This is the primary wealth generation mechanism.
     */
    function captureFounderWealth(uint256 grossYieldAmount) external onlyRole(STRATEGIST_ROLE) {
        require(founder != address(0), "Founder not set");
        uint256 fee = (grossYieldAmount * grossPerformanceFeeBps) / 10000;
        if (fee > 0) {
            SafeERC20.safeTransfer(IERC20(asset()), founder, fee);
        }
    }
