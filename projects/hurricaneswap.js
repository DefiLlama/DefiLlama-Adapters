...
async function staking() {
  const xHCT = retry(
    async (bail) =>
      await axios.post(
        "https://api.thegraph.com/subgraphs/name/hurricaneswap/farm",
        {
          query: `{
      pools(where: {id: "8"}){
        slpDeposited
  ...
  const price = retry(
    async (bail) =>
      await axios.post(
        "https://api.thegraph.com/subgraphs/name/hurricaneswap/exchange",
        {
          query: `{
          tokens(where:{
  ...
  const ratio = retry(
    async (bail) =>
      await axios.post(
        "https://api.thegraph.com/subgraphs/name/hurricaneswap/bar",
        {
          query: `{
      bars(first: 5) {
        id
  ...
  const xHCTTvl =
    (xHCT?.data?.data?.pools[0]["slpDeposited"] -
      xHCT?.data?.data?.pools[0]["slpWithdrawn"]) *
    hctprice *
    t;
