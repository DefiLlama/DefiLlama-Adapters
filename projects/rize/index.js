...
async function tvl(api) {
    ...
    const ethInScallopSui = await scallopSui("weth");
    ethTVL = ethTVL.plus(ethInScallopSui);
    const ethInNaviSui = await naviSui("weth");
    ethTVL = ethTVL.plus(ethInNaviSui);

    let usdtTVL = new BigNumber(0);
    const usdtInScallopSui = await scallopSui("usdt");
    usdtTVL = usdtTVL.plus(usdtInScallopSui);
    const usdtInNaviSui = await naviSui("usdt");
    usdtTVL = usdtTVL.plus(usdtInNaviSui);

    let usdcTVL = new BigNumber(0);
    const usdcInScallopSui = await scallopSui("usdc");
    usdcTVL = usdcTVL.plus(usdcInScallopSui);
    const usdcInNaviSui = await naviSui("usdc");
    usdcTVL = usdcTVL.plus(usdcInNaviSui);

    return {
        [`ethereum:${addresses.null}`]: ethTVL.times(1e18).toString(),
        [`ethereum:${addresses.ethereum.USDT}`]: usdtTVL.times(1e6).toString(),
        [`ethereum:${addresses.ethereum.USDC}`]: usdcTVL.times(1e6).toString(),
    }
}
...
async function scallopSui(coinSymbol) {
    ...
    const stakesBalance = new BigNumber(spoolAccountResp?.fields?.stakes ?? 0);
    const stakeBalance = stakesBalance.div(10 ** suiScallopPools[coinSymbol].decimals);
    const balanceSheetResp = await sui.getObject(suiScallopPools[coinSymbol].balanceSheet);
    const cash = new BigNumber(balanceSheetResp?.fields?.value?.fields?.cash ?? 0);
    const debt = new BigNumber(balanceSheetResp?.fields?.value?.fields?.debt ?? 0);
    const revenue = new BigNumber(balanceSheetResp?.fields?.value?.fields?.revenue ?? 0);
    const marketCoinSupply = new BigNumber(balanceSheetResp?.fields?.value?.fields?.market_coin_supply ?? 0);
    const conversionRate = cash.plus(debt).minus(revenue).div(marketCoinSupply);
    const tvl = stakeBalance.times(conversionRate).decimalPlaces(suiScallopPools[coinSymbol].decimals);
    return tvl;
}
...
async function naviSui(coinSymbol) {
    ...
    const currentSupply = new BigNumber(naviResp?.fields?.value ?? 0);
    ...
    const currentSupplyIndex = new BigNumber(currentSupplyIndexOrg).div(1e27);

    const decimals = suiNaviPools[coinSymbol].decimals;
    const tvl = currentSupply.div(1e9).times(currentSupplyIndex).decimalPlaces(decimals);
    return tvl;
}
