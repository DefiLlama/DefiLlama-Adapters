...
const checkToken = async (token, chain, api) => {
  let validToken = JSON.parse(fs.readFileSync(__dirname +'/validToken.json', 'utf8'));
  if (!validToken.hasOwnProperty(chain)) {
    validToken[chain] = [];
  }
  if (!validToken[chain].includes(token)) {
    try {
      const decimals = await api.call({ abi: abi.decimals, target: token })
      if (decimals) {
        validToken[chain].push(token);
      } else {
        return false
      }
    } catch {
  ...
  fs.writeFileSync(__dirname +'/validToken.json', JSON.stringify(validToken, null, 2));
...
const tvl = async (_, _1, _2, { api }) => {
  ...
  for (const [i, output] of pools.entries()) {
    tokenXValid = await checkToken(poolMetaData[i].tokenX, chain, api);
    tokenYValid = await checkToken(poolMetaData[i].tokenY, chain, api);
    if(tokenXValid && tokenYValid) {
      toa.push([poolMetaData[i].tokenX, output], [poolMetaData[i].tokenY, output]);
    }
  }
