...
async function getTokens(api) {
  ...
  const logs = await getLogs({
    ...
    target: lendingPool,
    topics: [
      "0x9f8f649e3f624ae845ed20c597f2841f852ba62903a53736c2b36d67869ca919",
    ],
    fromBlock,
  });
...
async function tvl(_, _b, _cb, { api }) {
  ...
  const bals = await api.multiCall({
    abi: abi.getTotalLiquidity,
    calls: tokens,
    target: lendingPool,
  });
  const borrows = await api.multiCall({
    abi: abi.getTotalDebt,
    calls: tokens,
    target: lendingPool,
  });
...
const abi = {
  getSupportedAsset:
    "function getSupportedAsset(uint256 _index) view returns (address)",
  getTotalDebt:
    "function getTotalDebt(address _underlyingAsset) view returns (uint256)",
  getTotalLiquidity:
    "function getTotalLiquidity(address _underlyingAsset) view returns (uint256)",
};
