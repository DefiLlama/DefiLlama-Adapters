...
const FACTORY_BSC_CONTRACT = '0xCe4529Fe88df480BD777d3e32dfD7032e6C685ff';
...
async function bscTvl(_, _1, _2, { api }) {
  const tokens = await api.fetchList({  lengthAbi: 'idSTOs', itemAbi: 'stoTokens', target: FACTORY_BSC_CONTRACT, startFromOne: true })
  const escrows = await api.fetchList({  lengthAbi: 'idSTOs', itemAbi: 'stoEscrows', target: FACTORY_BSC_CONTRACT, startFromOne: true })
  const supplies = await api.multiCall({  abi: 'erc20:totalSupply', calls: tokens})
  // const names = await api.multiCall({  abi: 'string:name', calls: tokens})
  const issuanceIndex = await api.multiCall({  abi: 'uint256:issuanceIndex', calls: escrows})
  const payTokens = await api.multiCall({  abi: 'address:paymentToken', calls: escrows})
