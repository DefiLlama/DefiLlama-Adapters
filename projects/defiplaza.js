...
const graphUrl =
  "https://api.thegraph.com/subgraphs/name/omegasyndicate/defiplaza";
...
module.exports = {
  ethereum: {
    tvl: async (timestamp, block, _, { api }) => {
      ...
      const v = api.sumTokens({ ownerTokens });
      console.log("Using v ", v);
      const t = await v;
      console.log("sing t ", t);
  ...
  radixdlt: {
    tvl: async (_, _1, _2, { api }) => {
        ...
        fetcher: async () => {
          ...
          do {
            const { data, next_cursor } = await get(
              `https://radix.defiplaza.net/api/pairs?cursor=${cursor}&limit=100`
            );
            items.push(...data);
      ...
      const data = await queryAddresses({
        addresses: pools.map((i) => i.address),
      });
      const owners = [];
      data.forEach((c) => {
        owners.push(
          c.details.state.fields.find((i) => i.field_name === "base_pool").value
        );
        owners.push(
          c.details.state.fields.find((i) => i.field_name === "quote_pool")
            .value
        );
      });
