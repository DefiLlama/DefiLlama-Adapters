...
async function tvl(timestamp, blockHeight, _2, { api }) {
  ...
  const baseTokens = await api.multiCall({
    ...
    calls: filteredLogs.map(i => i.pairAddress)
  });
  const filteredFarms = filteredLogs.filter(
    (i, idx) => (baseTokens[idx] = nullAddress)
  );
  ...
  const baseTokenAmounts = (
    await api.multiCall({
      abi: "function removeQuote(uint256) view returns (uint256,uint256)",
      calls: pairs.map((v, i) => ({ target: v, params: [totalSupplies[i]] }))
    })
  ).map(i => i[0]);
  const rewardBalances = await api.multiCall({
    abi: "erc20:balanceOf",
    calls: rewardTokens.map((v, i) => ({ target: v, params: [farms[i]] }))
  });
  baseTokenAmounts.forEach(i => api.add(nullAddress, i * 2));
  rewardBalances.forEach((v, i) =>
    api.add(nullAddress, (prices[i] * v) / 1e18)
  );
}
