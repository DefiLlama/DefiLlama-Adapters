...
async function smartQuery(contract, msgObj) {
  ...
  if (!res.ok) throw new Error(`wasm smart query failed: ${res.status}`)
...
async function bankBalances(address) {
  ...
  const res = await fetch(url)
  if (!res.ok) throw new Error(`bank balances failed: ${res.status}`)
  const json = await res.json()
...
async function tvl() {
  ...
  for (const key of ['ampLUNA', 'bLUNA', 'wSOL', 'wBNB']) {
    ...
    try {
      ...
      // skip on error
      // console.error('cw20 fetch error', key, e)
      continue
    }
  }
