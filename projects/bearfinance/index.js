...
async function tvl(timestamp, block, chainBlocks) {
  ...
  const bearInLiquidity = new BigNumber(
    balances[`fantom:${BEAR.toLowerCase()}`]
  );
  ...
  results[DAI.toLowerCase()] = new BigNumber(
    daiInLiquidity.plus(daiBalanceFromBear).plus(bearStakedInDai).toFixed(0)
  ).toString(10);
...
async function handleStaking(timestamp, _ethBlock, chainBlocks) {
  ...
  const results = {};
  results[DAI.toLowerCase()] = new BigNumber(
    daiInLiquidity.plus(bearStakedInDai).toFixed(0)
  ).toString(10);
  return results;
}
...
async function handlePool2(timestamp, _ethBlock, chainBlocks) {
  ...
  const results = {};
  results[DAI.toLowerCase()] = new BigNumber(
    daiInLiquidity.plus(daiBalanceFromBear).toFixed(0)
  ).toString(10);
  return results;
}
