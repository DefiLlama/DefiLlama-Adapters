 ...
 * Returns total USD value of collateral in RLP vaults.
 */
 ...
 * Vault collateral is denominated in USDC so we add it as tether equivalent.
 */
async function arbitrumTvl(api) {
  ...
  // Add Rocket Chain vault collateral (USD-denominated)
  const vaultTvl = await fetchRocketVaultTvl();
        if (vaultTvl > 0) {
                  const tetherKey = "tether";
                  balances[tetherKey] = (balances[tetherKey] || 0) + vaultTvl;
        }
