...
const perpsVolumeData = (
  await axios(perpsInfoApi)
);  


  let last24HourVolume = 0
  let fetchTimestamp = timestamp
  const globalOverview = perpsVolumeData.data.globalOverview
  ...
  if(globalOverview && globalOverview.daily_trading_volume.length > 1) {
    ...
    last24HourVolume = new BigNumber(globalOverview.daily_trading_volume[1].value).shiftedBy(-6)
    fetchTimestamp = Math.round(new Date(globalOverview.daily_trading_volume[1].date).getTime()/1000)
  }
