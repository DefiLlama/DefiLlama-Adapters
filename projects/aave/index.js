...
async function ammMarket(balances, block) {
  ...
  const balanceOfTokensWithoutBalancerPairs = (
    await Promise.all(
      balanceOfTokens.output.map(async (balanceOf, idx) => {
        ...
        balancerTokens.output[idx].output.forEach((token, ydx) => {
          const tokenBalance = BigNumber(amountsOnPair.output[ydx].output)
            .times(balanceOf.output)
  ...
  await addBalanceOfTokensAndLPs(
    balances,
    {
...
async function ethereum(timestamp, block) {
  ...
  let balances = await _multiMarketV1Tvl(
    aaveLendingPoolCore,
    aaveReserves,
    block
  );
  ...
  const uniswapv1Calls = Object.keys(uniswapMarketTvlBalances).map((t) => ({
    target: t,
  }));
  const [uniswapV1Tokens, uniswapV1EthBalance, uniswapV1Supplies] =
    await Promise.all([
      sdk.api.abi.multiCall({
        abi: {
          name: "tokenAddress",
  ...
  Object.keys(uniswapMarketTvlBalances).forEach((address, idx) => {
    ...
    if (uniswapV1Tokens.output[idx].success === false) {
      ...
      const tokenBalance = BigNumber(uniswapV1TokenBalance.output[idx].output)
        .times(balance)
        .div(uniswapV1Supplies.output[idx].output);
      const ethBalance = BigNumber(uniswapV1EthBalance.output[idx].balance)
        .times(balance)
        .div(uniswapV1Supplies.output[idx].output);
  ...
  if (block >= 11360925) {
    ...
    [v2Atokens, v2ReserveTokens, addressSymbolMapping] = await getV2Reserves(
      block,
      addressesProviderRegistryETH,
      "ethereum",
      v2Atokens,
      v2ReserveTokens,
      addressSymbolMapping
    );
