...
async function eos() {
  ...
  const lend = await utils.fetchURL(
    eosEndpoint + "lend/getGlobalOpenPositionStat"
  );
  const usn = await utils.fetchURL(eosEndpoint + "st/open/getGlobalOpenStat");
  const tvl =
    Number(swap.data.data.eosBalance) * eosPrice * 2 + // swap TVL
    lend.data.data.practicalBalance -
    lend.data.data.totalBorrowsVariable + // lend TVL
    usn.data.globalOpenStat.totalMortgage; // usn (stable token) TVL
  return tvl;
}

async function bsc() {
  const bnbPrice = (await utils.getPricesfromString("binancecoin")).data
    .binancecoin.usd;
  const swap = await axios.default.post(
    bscEndpoint + "swap/get24HInfo",
    {},
    { headers: { chainid: 56 } }
  );
  const tvl =
    swap.data.data.usd_balance + swap.data.data.wbnb_balance * bnbPrice; // swap TVL
  return tvl;
}
