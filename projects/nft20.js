...
eth = async (timestamp, block, chainBlocks) =>
  tvl(timestamp, block, chainBlocks, "ethereum");
polygon = async (timestamp, block, chainBlocks) =>
  tvl(timestamp, block, chainBlocks, "polygon");
...
async function tvl(timestamp, block, chainBlocks, chain) {
  ...
  const nft20_lp_usd_balance = nft20_pools.reduce(
    (acc, p) => parseFloat(p["lp_usd_balance"]) + acc,
    0
  );
  console.log(
    `${chain}: nft20 pools count: ${
      nft20_pools.length
    }\n${chain}: nft20 usd balance of LP returned by API: ${(
      nft20_lp_usd_balance / 1e6
    ).toFixed(2)}M`
  );
  ...
  const LPs = [...uni_v2_LPs, ...uni_v3_LPs].filter(
    (lp) => lp.output !== "0x0000000000000000000000000000000000000000"
  );
  const weth_LPs = await sdk.api.abi.multiCall({
    calls: LPs.map((pool) => ({
      target: weth,
      params: [pool.output],
    })),
