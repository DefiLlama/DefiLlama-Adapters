...
const polygonMasterChef =
  (masterChef, pid) => async (timestamp, block, chainBlocks) => {
    const balances = {};
...
const polygonTvl =
  ({ include, exclude }) =>
  async (timestamp, block, chainBlocks) => {
    const balances = {};
    ...
    vaults.forEach((v, idx) => {
      if (
        ...
        (lp_symbols[idx] === "crvUSDBTCETH") |
        (lp_symbols[idx] === "am3CRV") |
        (lp_symbols[idx] === "btcCRV")
      ) {
        crvPositions.push({
          vaultAddr: vaults[idx],
          balance: vault_balances[idx],
...
const fantomTvl = async (timestamp, block, chainBlocks) => {
  ...
  vaults.forEach((vault) => {
    ...
    // CRV
      ...
      ftm_CrvVaultAddr.includes(vault.vault) |
      String(vault.__comment).toLowerCase().includes("curve ")
    ) {
