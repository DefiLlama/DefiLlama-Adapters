...
const NARAUSD = '0x5C6263904CCFD3Bcf1aAa6e7063dDd29743b3Bb7' // NaraUSD stablecoin
const NARAUSD_PLUS = '0x1aa23CDFC941f6b54251C72012A9Bfa4bF5394D6' // NaraUSD+, Staked NaraUSD, yield bearing 

const COLLATERAL_OWNERS = [COLLATERAL_SAFE, MCT, NARAUSD]

// Collateral counted at face value. Only assets that are $1-denominated in their own right are listed here; every yield-bearing position below is unwrapped to its underlying with an on-chain rate instead, so nothing depends on DefiLlama holding a price for a wrapper token.
const COLLATERAL_TOKENS = [ADDRESSES.ethereum.USDT, ADDRESSES.ethereum.USDC]

// sUSDS is an ERC4626 vault over USDS. convertToAssets turns the share balance into the USDS it is redeemable for, which is the real value of the position.
const SUSDS = ADDRESSES.ethereum.sUSDS
const USDS = ADDRESSES.ethereum.USDS

// Midas Fasanara Global. Valued from Midas' own price feed rather than left to generic pricing, so the number is exact and independent of whether DefiLlama tracks this token.
const MGLOBAL = '0x7433806912Eae67919e66aea853d46Fa0aef98A8'
const MGLOBAL_DECIMALS = 18n
const MGLOBAL_FEED = '0x66Aa9fcD63DF74e1f67A9452E6E59Fbc67f75E38' // MGlobal Price Oracle

// Blackopal LiquidStone II: a BoringVault share (6dp) with no market price, so it is unwrapped to its base asset with the vault accountant on-chain rate.
...
// getRate() is 18-decimal (verified on-chain: 1.1358e18 at 2026-08-26), while the share token is 6-decimal, so the product is scaled back down by 1e18 and the result is already in the 6-decimal base asset
const RATE_SCALE = 10n ** 18n

// MCT is a claim on the same collateral counted above, so it must never be added itself. The rest are unsolicited airdrops sent to protocol addresses.
const EXCLUDED = [
  MCT,
...
module.exports = {
  // Positions with no DefiLlama price - Midas Fasanara Global, the BlackOpal LS2 vault share, and NaraUSD itself - are unwrapped to their underlying value and reported against a priced stable, so the token breakdown shows the proxy asset rather than the original.
  ...
  ethereum: {
    tvl: async api => {
      ...
      // MGLOBAL (18dp) * feed answer -> value denominated in the 6dp base asset. Feed decimals are read on-chain as well.
    ...
    staking: async api => {
      // NaraUSD has no price on DefiLlama yet, so summing it directly returns zero. It is fully collateralised 1:1 by the assets counted in tvl above, so it is represented here by USDC for valuation only. NaraUSD is 18dp, USDC 6dp.
