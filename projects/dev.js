...
async function fetch() {
  const addressConfigInstance = await new web3.eth.Contract(JSON.parse(ADDRESS_CONFIG_ABI), ADDRESS_CONFIG_ADDRESS)
  const lockupAddress = await addressConfigInstance.methods.lockup().call()
  const lockupInstance = await new web3.eth.Contract(JSON.parse(LOCKUP_ABI), lockupAddress)
