...
// async function tvl(api) {
//   const v3Targets = orderbooks[api.chain].v3.map(v => v.address)
//   const v4Targets = orderbooks[api.chain].v4.map(v => v.address)
//   if (!v3Targets.length && !v4Targets.length) return

//   const fromTimestamp = Object.values(orderbooks[api.chain])
//     .flat()
...
//       chain: api.chain,
//       toTimestamp: api.timestamp,
//       targets: [v3Targets, v4Targets].flat(),
//       topic: id(abi_v3.getEvent("Deposit").format()),
//     }),
//   ]
//   if (v3Targets.length) {
//     promises.push(getEventLogs({
//       fromTimestamp,
//       flatten: false,
//       entireLog: true,
//       chain: api.chain,
...
//     promises.push((async() => [])())
//   }
//   const [depositLogs, addOrderLogs, addOrderV2Logs] = await Promise.all(promises)

//   depositLogs?.forEach(orderbookLogs => {
...
//     })
//   })
//   addOrderLogs?.forEach(orderbookLogs => {
//     orderbookLogs.forEach(log => {
//       if (!tokenMap[log.address]) tokenMap[log.address] = []
//       const {
//         order: { validInputs, validOutputs }
...
//     })
//   })
//   addOrderV2Logs?.forEach(orderbookLogs => {
//     orderbookLogs.forEach(log => {
//       if (!tokenMap[log.address]) tokenMap[log.address] = []
//       const {
//         order: { validInputs, validOutputs }
