...
async function tvl(timestamp, block, chainBlocks, { api }) {
  const logs = await getLogs({
    ...
    topics: [
      "0x489ab9065c597368f4a678fadcb323bf4c848713ea7d5a296d16ec97203eae83",
    ],
    eventAbi:
      "event StakingPoolDeployed(address indexed poolAddress,address indexed stakingToken,uint256 startTime,uint256 roundDurationInDays)",
    onlyArgs: true,
    fromBlock: 17015686,
  });

  const lsdAddresses = logs.map((i) =>
    i.stakingToken === nullAddress ? WETH_ADDRESS : i.stakingToken
  );
  ...
  return sumTokens2({
    ...
    blacklistedTokens: [AGI_ETH_LP_ADDRESS],
  });
}
...
module.exports = {
  ethereum: {
    ...
    tvl,
    pool2: pool2s([AGI_ETH_STAKING_POOL], [AGI_ETH_LP_ADDRESS]),
  },
};
