...
async function getPools(poolsJsonUrl) {
  return poolsJsonUrl === "local"
    ? require("./v2/legacy-pools.json")
    : (await axios.get(poolsJsonUrl)).data;
}
...
async function tvlV2Onchain(block, chain) {
  ...
  await unwrapCreamTokens(
    balances,
    safebox.map((s) => [s.cyTokenAddress, s.safeboxAddress]),
    block,
    chain,
    transform
  );
  ...
  stakingPoolsLpTokens.forEach((amount, i) =>
    lpPools.push({
...
async function tvlV2(block, chain) {
  const {
    safeBoxApi,
    coreOracleAddress,
    latestAlphaHomoraV2GraphUrl,
    instances,
  } = chainParams[chain];
  const cyTokens = await getCyTokens(
    block,
    safeBoxApi,
    latestAlphaHomoraV2GraphUrl
  );
  const collateralGroups = await Promise.all(
    instances.map((params) => getTotalCollateral(block, params))
  );
  ...
  const totalCollateralValue = BigNumber.sum(
    ...collateralGroups.map((collaterals) =>
      sumCollaterals(collaterals, tokenPrices)
    )
  );
