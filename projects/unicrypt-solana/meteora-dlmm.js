...
async function getDlmmBinArraysByPair(pairIds) {
...
async function addMeteoraLocks(api) {
  ...
  const [positionsRaw, lbPairsRaw, binArraysByPair] = await Promise.all([
    ...
    getDlmmBinArraysByPair(lbPairIds),
  ])
  ...
  for (const lock of locks) {
    ...
    const arrays = binArraysByPair.get(lock.lbPair) || []
    const binsById = new Map()
    arrays.forEach((arr) => {
      const map = parseBinArrayState(arr)
      map.forEach((v, k) => binsById.set(k, v))
    })
