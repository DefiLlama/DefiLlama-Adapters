...
function getMirLiquidity(network) {
  return request(graphUrl, mirLiquidityQuery, {
    network,
  }).then((data) => data.assets);
}

const getTotalLiq = (assets) =>
  assets.reduce((acc, asset) => acc + ust(asset.statistic.liquidity), 0);
const getPool2Liq = (liq) =>
  ust(liq.find((asset) => asset.symbol === "MIR").statistic.liquidity);
...
async function terraTvl() {
  const totalTvl = request(graphUrl, tvlQuery).then((data) =>
    ust(data.statistic.totalValueLocked.total)
  );
