...
Object.keys(config).forEach(chain => {
  ...
  module.exports[chain] = {
    tvl: async (_, _b, _cb, { api, }) => {
      ...
      const strategy = logs.map(i => i.args.strategy)
      const principal = logs.map(i => i.args.principal)
      let bals = await api.multiCall({  abi: 'uint256:getPrincipalBalance', calls: strategy}) 
      bals.forEach((bal, i) => sdk.util.sumSingleBalance(balances,principal[i],bal, api.chain))

      bals = await api.multiCall({  abi: 'uint256:getYieldBalance', calls: strategy}) 
      bals.forEach((bal, i) => sdk.util.sumSingleBalance(balances,principal[i],bal, api.chain))
      return balances
    }
  }
