...
// locked (burned) into the vault's vote-escrow NFT, so only the vault's ETH income and
// not-yet-locked SLVR balance are counted.
...
async function staking(api) {
  // Staking counts only real SLVR balances: withdrawable locks (held by the vote escrow), 
  // game contract balances (pending claims/emissions) and the liSLVR vault's deposits pending lock.
  return api.sumTokens({ owners: [...await gameContracts(api), VOTE_ESCROW, LISLVR_VAULT], tokens: [SLVR] })
}
