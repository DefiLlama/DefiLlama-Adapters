...
async function getBalances(timestamp, block, chainBlocks, network) {
  ...
  iTokens.forEach((iToken) => {
    const supply = supplyResult.output.find(
      (result) => result.input.target === iToken.iTokenAddress
    );
    const borrow = borrowResult.output.find(
      (result) => result.input.target === iToken.iTokenAddress
    );
  ...
  if (contracts[network].staking) {
    const calls = [];
    balanceOfCalls.push({
      target: tokens[network].pool3,
      params: contracts[network].bzx,
    });
    balanceOfCalls.push({
    ...
    balanceOfCalls.push({
      target: tokens[network].lp,
      params: contracts[network].bzx,
    });
    balanceOfCalls.push({
      target: tokens[network].lp_old,
      params: contracts[network].bzx,
    });
    balanceOfCalls.push({
      target: tokens[network].bzrx,
      params: contracts[network].staking,
    });
    balanceOfCalls.push({
      target: tokens[network].vbzrx,
      params: contracts[network].staking,
    ...
    for (let i = 0; i < calls.length; i++) {
      ...
      balances[token] = BigNumber(balances[token] || 0)
        .plus(BigNumber(supplyByAsset.output[i].output))
        .toFixed();
    }
    ...
    if (contracts[network].sushi_masterchef) {
      ...
      balances[token] = BigNumber(balances[token] || 0)
        .plus(BigNumber(masterchefSupply.output.amount))
        .toFixed();
    }
  }
  ...
  for (let i = 0; i < balanceOfCalls.length; i++) {
    ...
    if (call.target) {
      ...
      balances[token] = BigNumber(balances[token] || 0)
        .plus(BigNumber(balanceOfs.output[i].output))
        .toFixed();
    }
  }

  //Unwrap lp
  const lps = Object.keys(tokens[network])
    .filter((key) => key.indexOf("lp") >= 0)
    .map((key) => {
      return {
  ...
  if (tokens[network].vbzrx) {
    ...
    balances[tokens[network].bzrx] = BigNumber(
      balances[tokens[network].bzrx] || 0
    )
      .plus(BigNumber(balances[tokens[network].vbzrx] || 0 * vbzrxWorthPart))
      .toFixed();
