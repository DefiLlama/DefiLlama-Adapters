...
const graphUrl = "https://api.thegraph.com/subgraphs/name/visorfinance/visor";
...
async function tvlLiquidityMining(timestamp, block) {
  ...
  const resp = await request(graphUrl, graphQuery);
...
async function tvlUniV3(timestamp, block) {
  ...
  //   })
  // ).output
  const resp = await request(graphUrl, uniV3HypervisorQuery);
