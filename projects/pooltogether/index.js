...
const celoGraphUrl =
  "https://api.thegraph.com/subgraphs/name/pooltogether/celo-v3_4_5";
const bscGraphUrl =
  "https://api.thegraph.com/subgraphs/name/pooltogether/bsc-v3_4_3";
...
const v4pools = {
  ...
  polygon: [
    [
      ...
      "0xD4F6d570133401079D213EcF4A14FA0B4bfB5b9C",
    ],
  ],
};
...
async function eth(timestamp, block) {
  ...
  for (const graphUrl of graphUrls) {
    const { prizePools } = await request(graphUrl, graphQuery, {
      block,
    });
  ...
  allPrizePools = [
    ...new Set(combinedPrizePools.map((a) => JSON.stringify(a))),
  ].map((a) => JSON.parse(a));
...
async function celo(timestamp, block, chainBlocks) {
  ...
  const { prizePools } = await request(celoGraphUrl, graphQuery, { block });
  allPrizePools = allPrizePools.concat(prizePools);
