...
async function tvl(api) {
  ...
  if (chain === "ethereum") {
    ...
    assets.forEach((output, i) => sdk.util.sumSingleBalance(balances, tokens[i], output));
    ...
    const eurocBalance =
      eurocBal * (+sdagEUREUROCTVL + +cvxagEUREUROCstakerTVL) / totPoolTokenSupply;
    sdk.util.sumSingleBalance(balances, EUROC, eurocBalance);

    //Fetch all collaterals from transmuters
    const transmutersCollaterals = (await Promise.all(
      stablecoins.map(({ transmuter, genesis }) => transmuterCollaterals(api, transmuter, genesis))
    )).flat(1);
