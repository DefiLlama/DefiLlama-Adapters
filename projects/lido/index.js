...
async function terra(timestamp, ethBlock, chainBlocks) {
  ...
  */
  const { total_bond_amount } = (
    await axios.get(
      `https://lcd.terra.dev/wasm/contracts/terra1mtwph2juhj0rvjz7dy92gvl6xvukaxu8rfv8ts/store?query_msg=%7B%22state%22%3A%20%7B%7D%7D`
    )
  ).data.result;
  return {
...
async function solana(timestamp, ethBlock, chainBlocks) {
  ...
  const reserveAccountBalance = await sol.retrieveReserveAccountBalance(
    connection
  );
