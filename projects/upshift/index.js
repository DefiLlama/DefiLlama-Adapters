...
const suiVaultsEndpoint = "https://vaults.api.sui-prod.bluefin.io/api/v1/vaults";
...
const suiVaultsTvl = async () => {
  const vaults = await getConfig('upshift/suiVaults',suiVaultsEndpoint);
  const totalTvl = vaults.filter(vault => vault.status === "active").reduce((acc, vault) => {
    acc += vault.totalDepositsInUsdE9 / 1e9;
    return acc;
  }, 0);
  return {
    usd: totalTvl,
