...
async function fetchTotalSupply() {
  const totalSupplyResponse = await retry(
    async (bail) =>
      await axios.get(totalSupply, {
        headers: {
          "x-signature":
...
async function ethereum() {
  const price_feed = await retry(
    async (bail) => await axios.get(btcPriceEndPoint)
  );
  const totalSupplyResponse = await fetchTotalSupply();
  return new BigNumber(totalSupplyResponse["erc20TotalSupply"])
    .multipliedBy(new BigNumber(price_feed.data.bitcoin.usd))
    .toFixed(2);
}
...
async function polygon() {
  ...
  const totalSupplyResponse = await fetchTotalSupply();
  return new BigNumber(totalSupplyResponse["mrc20TotalSupply"])
    .multipliedBy(new BigNumber(price_feed.data.bitcoin.usd))
    .toFixed(2);
}
