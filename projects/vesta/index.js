...
async function tvl(_, block, chainBlocks) {
  ...
  output.forEach(({ input: { params: [token] }, output }) => {
    ...
    const llamaTokenAddress = transform(token.toLowerCase() === VaultTokens.GLP ? '0x4277f8F2c384827B5273592FF7CeBd9f2C1ac258' : token) // convert sGLP to feGLP address for price api
