...
async function getOldToA(api) {
  const data = await getConfig('lyra-v2/old-contracts', 'https://raw.githubusercontent.com/0xdomrom/socket-plugs/feat/lyra-base/deployments/superbridge/prod_lyra-old_addresses.json')
...
async function getToA(api) {
  const data = await getConfig('lyra-v2/contracts', 'https://raw.githubusercontent.com/0xdomrom/socket-plugs/feat/lyra-base/deployments/superbridge/prod_lyra_addresses.json')
  const vaults = Object.values(data[api.chainId+'']?? {}).map(i => i.Vault)
