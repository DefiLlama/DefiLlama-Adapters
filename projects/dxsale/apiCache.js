...
function getTVLTotal(args) {
    return async (timestamp, ethBlock, chainBlocks) => {
        ...
        async function addlockLPs(lockContract) {
            if (!cache.lockContracts[lockContract]) cache.lockContracts[lockContract] = {
                walletIds: [],
                tokens: [],
                walletConfig: {}
            }
            ...
            const tempBalances = await vestingHelper({
                cache,
        ...
        async function addV3Lps() {
            ...
            for (let i = 0; i < args.storageLiquidityLocks.length; i++) {
                ...
                lpData.forEach(({
                                ...
                                }) => cache.v3LPData.push([lockedLPTokens, lpLockContract]))
            }

            const tempBalances = await sumUnknownTokens({
                chain,
                block,
                tokensAndOwners: cache.v3LPData,
                useDefaultCoreAssets: true,
                cache,
            })
