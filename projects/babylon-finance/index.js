...
async function tvl(timestamp, ethBlock, chainBlocks) {
  ...
  for (const gardenDetails of gardensDetails.output) {
    ...
    const strategies_tvl = strategiesDetails.output.map((strategyDetails) => {
      ...
      strategy_str += `   strategy ${strategyName}: nav (tvl): ${
        nav / 1e18
      } out of capitalAllocated, : ${capitalAllocated / 1e18}\n`;
      return nav;
    });

    const garden_tvl = strategies_tvl.reduce(
      (acc, el) => acc.plus(BigNumber(el)),
      BigNumber(garden_idle)
    );
    balances[reserveAsset] = balances[reserveAsset]
      ? BigNumber(balances[reserveAsset]).plus(garden_tvl)
      : garden_tvl;
    if (garden_tvl / 1e18 > 10)
      console.log(
        `Garden with name "${gardenName}" TVL: ${
          garden_tvl / 1e18
        } of reserveAsset: ${reserveAsset} locked\n${strategy_str}-----------------------------`
      );
  }
  return balances;
}
...
module.exports = {
  methodology:
    "TVL of Babylon corresponds to capital locked into each garden (idle capital waiting to be deployed) as well as capital deployed to each strategy of these gardens",
  ethereum: {
    staking: () => ({}),
    tvl,
  },
