...
async function getIcxUsdPrice() {
  let response = await icxCall(bandOracleContract, "get_ref_data", {
    _symbol: "ICX",
  });
...
async function getDexTvl() {
  ...
  let sicxIcxPool = await icxCall(balancedDexContract, "totalSupply", {
    _id: "1",
  }); // Get sICX/ICX TVL in ICX.
  let sicxBnusdPool = await icxCall(balancedDexContract, "getPoolTotal", {
    _id: "2",
    _token: bnusdContract,
  }); // Get sICX/bnUSD TVL in bnUSD.
  let balnBnusdPool = await icxCall(balancedDexContract, "getPoolTotal", {
    _id: "3",
    _token: bnusdContract,
  }); // Get BALN/bnUSD TVL in bvnUSD.
  let balnSicxPool = await icxCall(balancedDexContract, "getPoolTotal", {
    _id: "4",
    _token: sicxContract,
  }); // Get BALN/sICX TVL in sICX.

  let sicxIcxTvlUsd = (parseInt(sicxIcxPool, 16) * icxUsdPrice) / 10 ** 18;
  let sicxBnusdTvlUsd =
    (parseInt(sicxBnusdPool, 16) * 2 * bnusdUsdPrice) / 10 ** 18; // Multiply by 2 to get USD value of entire pool.
  let balnBnusdTvlUsd =
    (parseInt(balnBnusdPool, 16) * 2 * bnusdUsdPrice) / 10 ** 18; // Multiply by 2 to get USD value of entire pool.
  let balnSicxTvlUsd =
    (parseInt(balnSicxPool, 16) * 2 * sicxUsdPrice) / 10 ** 18; // Multiply by 2 to get USD value of entire pool.

  let dexTvlUsd =
    sicxIcxTvlUsd + sicxBnusdTvlUsd + balnBnusdTvlUsd + balnSicxTvlUsd;
...
module.exports = {
  Methodology:
    'TVL consists of liquidty on the DEX and deposits made to the lending program. Data is pulled from the ICX API "https://ctz.solidwallet.io/api/v3"',
  fetch,
};
