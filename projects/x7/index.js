...
async function getTreasury(timestamp, block, chainBlocks) {
    const balances = {};
    const totalTreasury = await sdk.api.eth.getBalances({
        targets: [
            ethTreasury.X7TreasurySplitterAddress,
            ethTreasury.DeveloperMultiSigWalletAddress,
            ethTreasury.CommmunityMultiSigWalletAddress,
            ethTreasury.MarketingWalletAddress,
            ethTreasury.DeveloperWalletAddress,
        ],
        chain: 'ethereum',
        block,
    });

    balances[`ethereum`] = totalTreasury.output.reduce((total, item) =>
        BigNumber(item.balance / 1e18).plus(total), 0).toFixed(0);
