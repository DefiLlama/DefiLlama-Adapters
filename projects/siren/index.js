...
async function calculateMainnetTVL(timestamp, block, chainBlocks) {
  ...
  for (let i = 0; i < mainnet_amms["amms"].length; i++) {
    ...
    mainnet_result[collateralToken] = BigNumber(mainnet_result[collateralToken])
    ...
    for (let im = 0; im < amm.markets.length; im++) {
      ...
      mainnet_result[collateralToken] = BigNumber(
        mainnet_result[collateralToken]
      )
        .plus(response.output)
        .toFixed();
    }
...
async function calculatePolygonTVL(timestamp, block, chainBlocks) {
  const { seriesVaults } = await request(
    POLYGON_GRAPH_URL,
    GET_SERIES_VAULT_ID
  );
  ...
  for (let i = 0; i < polygon_amms["amms"].length; i++) {
    ...
    polygon_result["polygon:" + collateralToken] = BigNumber(
      polygon_result["polygon:" + collateralToken]
    )
      .plus(response.output)
      .toFixed();
