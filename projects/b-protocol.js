...
async function runMaker() {
  const bCdpManager = new web3.eth.Contract(
    bCdpManagerAbi,
    "0x3f30c2381CD8B917Dd96EB2f1A4F96D91324BBed"
  );
  const vat = new web3.eth.Contract(
    vatAbi,
...
async function runCompound() {
  const userInfo = new web3.eth.Contract(
    userInfoAbi,
    "0x907403da04eb05efd47eb0ba0c7a7d00d4f233ea"
  );
  ...
  const info = await userInfo.methods
    .getUserInfo(
      user,
      comptroller,
      bComptrollerAddress,
  ...
  for (let i = 0; i < info.tokenInfo.ctoken.length; i++) {
    ...
    const cExchangeRate = new web3.utils.toBN(
      info.tokenInfo.ctokenExchangeRate[i]
    );
    ...
    const usdBalance = web3.utils.fromWei(
      underlyingBalance.mul(price).div(_1e18)
    );
