...
const config = {
  ...
  base: {
    ...
    tokensAndOwners: [
      //      Token                           Potion(lToken)                                  slToken
      [ADDRESSES.base.WETH, '0x0540f15374eCF13aAB3c0a76b643333CE0D00579', '0x0E7De1d6A1aA4178CBfce3dE4EAaD0427034f924'],
...
async function tvl( api, coveredAssets, result ={} ){
  for(const assets of coveredAssets){
    ...
    const originalTokenDecimals = Number( 
      await api.call({
        abi : 'erc20:decimals',
        target : originalToken,
      })
    ...
    const tokenBalanceOnPotion = adjustAmountByDecimals(Number(balances[0]), originalTokenDecimals)
    const ltokenBalanceOnSLToken = adjustAmountByDecimals(Number(balances[1]), 18)
    potionSupply = adjustAmountByDecimals(potionSupply, 18)
    ...
    result[originalToken] = totalTokenBalance
  }
  return result
} 

function adjustAmountByDecimals( amount, decimals){
  ...
  module.exports[chain] = {
    ...
    staking: async (api) => {
      ...
      if (ownTokensAndOwners){
        ...
        return api.sumTokens({ tokensAndOwners: ownTokensAndOwners })
      }
    },
  }
})