...
const poolInfo = {
  ...
  outputs: [
    ...
    { internalType: "uint256", name: "accArenaPerShare", type: "uint256" },
...
async function staking(timestamp, block, chainBlocks) {
  ...
  for (let i in pyramStakingBalances) {
    sdk.util.sumSingleBalance(
      balances,
      `bsc:${pyramStakingBalances[i].input.target}`,
      pyramStakingBalances[i].output
    );
  }
  return balances;
}
