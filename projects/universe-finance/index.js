...
const vaultsUrl =
  "https://raw.githubusercontent.com/UniverseFinance/UniverseFinanceProtocol/main/doc/vaultAddress.json";
...
async function tvl(timestamp, block) {
  ...
  let allVaults = resp.data
    .filter((vault) => vault.type > 0)
    .map((vault) => ({
  ...
  for (let i = 0; i < typeNumber; i++) {
    ...
    for (let i = 0; i < addressList.length; i++) {
      ...
      sdk.util.sumSingleBalance(
        balances,
        token0[i].output,
        totalAmount[i].output[addressList[i].index]
      );
      sdk.util.sumSingleBalance(
        balances,
        token1[i].output,
        totalAmount[i].output[addressList[i].index + 1]
      );
    }
  }
...
module.exports = {
  methodology:
    "Vault TVL consists of the tokens in the vault contract and the total amount in the UNI V3 pool through the getTotalAmounts ABI call",
  ethereum: {
    tvl,
  },
  tvl,
