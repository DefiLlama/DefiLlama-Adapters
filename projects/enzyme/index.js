...
async function tvl(timestamp, block) {
  ...
  for (let i = 0; i < derivatives.length; i++) {
    ...
    results.forEach((asset) => {
      if (
        asset.networkAssetHolding === null ||
        asset.networkAssetHolding.amount === "0"
      ) {
        return;
      }
      let token;
        ...
        case "Synthetix":
          ...
          // If sXTZ convert to regular Tezos
            ...
            sdk.util.sumSingleBalance(
              balances,
              ["tezos"],
              BigNumber(asset.networkAssetHolding.amount).toFixed(0)
            );
            return;
          }
          decimals = asset.decimals;
          balance = BigNumber(asset.networkAssetHolding.amount)
        ...
        case "CurvePool":
          ...
          if (token === "0x3c0ffff15ea30c35d7a85b85c0782d6c94e1d238") {
            sdk.util.sumSingleBalance(
              balances,
              ["ethereum"],
              BigNumber(asset.networkAssetHolding.amount).div(2).toFixed(0)
            );
            sdk.util.sumSingleBalance(
              balances,
              "0x5e74c9036fb86bd7ecdcb084a0673efc32ea31cb",
              BigNumber(asset.networkAssetHolding.amount)
                .times(10 ** 18)
                .div(2)
                .toFixed(0)
            );
...
async function staking(timestamp, block) {
  ...
  `;
  let balance = (await request(queryUrl, field, { block })).asset
    .networkAssetHolding.amount;
  sdk.util.sumSingleBalance(
    balances,
    mln,
    BigNumber(balance)
...
module.exports = {
  methodology:
    "Uses subgraph to retrieve asset information and balances then uses onchain calls to calculate TVL.",
