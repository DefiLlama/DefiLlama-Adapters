...
function makeTvlFn(chain) {
  return async () => {
    ...
    const hostZoneUrl = chain.chainId === "dymension_1100-1"
        ? "https://stride-strd-api.polkachu.com/Stride-Labs/stride/stakedym/host_zone"
        : `https://stride-strd-api.polkachu.com/Stride-Labs/stride/stakeibc/host_zone/${chain.chainId}`;

    const [{ amount: assetBalances }, { host_zone: hostZone }] =
      await Promise.all([
        await get(
          `https://stride-strd-api.polkachu.com/cosmos/bank/v1beta1/supply/by_denom?denom=st${chain.denom}`
        ),
