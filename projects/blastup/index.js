...
async function stakingBlastUpTvl(_, block, _cb, { api }) {
  const balances = {};
  ...
  return transformBalances("blast", balances);
}

async function tvl(...args) {
  const blastUpStakingBalances = await stakingBlastUpTvl(...args);
  const stakingBalances = await stakings(
    [stakingpool],
    [ADDRESSES.blast.USDB, ADDRESSES.blast.WETH]
  )(...args);

  sdk.util.mergeBalances(stakingBalances, blastUpStakingBalances);
...
module.exports = {
  ...
  hallmarks: [
    ...
    [1718871654, "Staking LockedBlastUP Launch"],
    [1724142054, "Staking BlastUP Launch"],
  ],
};
