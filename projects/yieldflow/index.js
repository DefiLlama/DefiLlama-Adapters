...
async function tvl(api) {
    ...
    if (api.chain === 'ethereum') {
        usedManagers = ethManagers;
    }
    if (api.chain === 'arbitrum') {
        usedManagers = arbManagers;
    }

    const positionManagers = await api.multiCall({abi: 'address:positionManager', calls: usedManagers})
    const tokenIds = await api.multiCall({abi: abi.getCurrentTokenId, calls: usedManagers})
    const liquidities = await api.multiCall({
        abi: abi.getPositions,
        calls: positionManagers.map((v, i) => ({target: v, params: tokenIds[i]}))
    })
    const tokenAmounts = await api.multiCall({
        abi: abi.getAmountsForTicks,
        calls: liquidities.map((v, i) => ({target: usedManagers[i], params: [v.tickLower, v.tickUpper, v.liquidity]}))
    })
