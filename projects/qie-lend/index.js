...
async function tvl(timestamp, block, chainBlocks) {
    ...
    for (let i = 0; i < ctokens.length; i++) {
        ...
        // Handle totalBorrows safely
        const borrowRaw = borrowRes.output[i]?.success
            ? borrowRes.output[i].output
            : "0"


        // Safely convert values to full integer strings
        const cash = new BigNumber(cashRaw).toFixed(0)
        const borrows = new BigNumber(borrowRaw).toFixed(0)
