...
async function tvl(timestamp, block, chainBlocks) {
  ...
  console.log(
    totalSupplies.output
      .map(
        (call) =>
          call.input.target + ", " + BigNumber(call.output).div(1e6).toFixed(2)
      )
      .join("\n")
  );
  ...
  totalAtom = BigNumber(totalSuppliesOut[0].output).plus(
    BigNumber(totalSuppliesOut[1].output)
  );
  totalXPRT = BigNumber(totalSuppliesOut[2].output).plus(
    BigNumber(totalSuppliesOut[3].output)
  );
  balances = {
