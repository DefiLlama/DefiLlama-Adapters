...
async function getDonkeySwapRatio() {
  return axios
    .post("https://api.thegraph.com/subgraphs/name/uniswap/uniswap-v3", {
      query:
        '{ pool(id: "0xe73fe82f905e265d26e4a5a3d36d0d03bc4119fc") { token0Price } }',
    })
...
async function staking() {
  ...
  const priceOracleContract = new web3.eth.Contract(
    PriceOracleAbi,
    PriceOracleAddress.ethereum
  );
  const { 0: symbols, 1: prices } = await priceOracleContract.methods
    .getPrices()
    .call();
...
async function ethereum() {
  ...
  const controllerContrat = new web3.eth.Contract(
    ControllerAbi,
    controllerAddress.ethereum
  );
  ...
  for (let i = 0; i < ethMarkets.length; i++) {
    ...
    if (marketAddress === mainDTokenAddress.ethereum) {
      ...
      const underlyingDecimals = await underlyingContract.methods
        .decimals()
        .call();
      const cash =
        (await contract.methods.getCash().call()) / 10 ** underlyingDecimals;
...
async function klaytn() {
    ...
    headers: [
      {
        ...
        value: `Basic ${Buffer.from(
          `${KLAYTN_ACCESS_KEY_ID}:${KLAYTN_SECRET_ACCESS_KEY}`
        ).toString("base64")}`,
      },
  ...
  const klaytnPriceOracleContract = new caver.klay.Contract(
    PriceOracleAbi,
    PriceOracleAddress.klaytn
  );
  const { 0: klaytnSymbols, 1: klaytnPrices } =
    await klaytnPriceOracleContract.methods.getPrices().call();
  ...
  for (let i = 0; i < klaytnMarkets.length; i++) {
    ...
    if (marketAddress === mainDTokenAddress.klaytn) {
      ...
      const underlyingAddress = await contract.methods.underlying().call();
      const underlyingContract = new caver.klay.Contract(
        IERC20Abi,
        underlyingAddress
      );
      const underlyingDecimals = await underlyingContract.methods
      ...
      const underlyingSymbol = await contract.methods.underlyingSymbol().call();
      const tvlKrw =
        cash * klaytnPriceObj[Web3.utils.hexToUtf8(underlyingSymbol)];
      totalTvlKrw += tvlKrw;
    }
  }
  return totalTvlKrw / currency;
