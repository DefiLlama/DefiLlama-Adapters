...
async function lpStakingTvl(chain, meta, ethBlock) {
  ...
  for (let i = 0; i < meta.underlying.length; i++) {
    ...
    const underlyingTvl = poolSupplyBN
      .multipliedBy(underlyingLpBalanceBN)
      .div(lpTotalSupplyBN);
  ...
  for (const staking of stakingInfo[chain]) {
    ...
    if (typeof stakingTvl === "string") {
      sdk.util.sumSingleBalance(
        tvl,
        transform(staking.meta.tokenAddress),
        stakingTvl
      );
    } else {
...
module.exports = {
  ...
  bsc: {
    ...
    pool2: pool2(
      ...
      "0x72ba008B631D9FD5a8E8013023CB3c05E19A7CA9",
      "bsc"
    ),
    tvl: bscTvl,
  },
