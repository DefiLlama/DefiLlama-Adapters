const { queryContractStore, queryV1Beta1, endPoints } = require('../helper/chain/cosmos')
const { queryContract, getContracts, } = require('../helper/chain/secret')
const secret = require('secretjs')
const { transformDexBalances } = require('../helper/portedTokens')

async function tvl(_, _b, _cb, { api, }) {
  // const contracts = await getContracts(361)
  // console.log(contracts)
  const factiories = ["secret18sq0ux28kt2z7dlze2mu57d3ua0u5ayzwp6v2r"]
  // const factiories = ["secret1zvk7pvhtme6j8yw3ryv0jdtgg937w0g0ggu8yy", 'secret18sq0ux28kt2z7dlze2mu57d3ua0u5ayzwp6v2r']
  const data = (await Promise.all(factiories.map(getExchanges))).flat()
  return transformDexBalances({ data, chain: api.chain })
}

async function getExchanges(factory) {
  const pools = []
  const limit = 25
  let hasMore = true
  const a = new secret.ComputeQuerier(endPoints.secret)
  const testRes = await a.queryContract({ contract_address: 'secret1lqkzf83zjzve9jtv7z8sf40wn4khpaem097qvp', query: { pair_info: { } } })
  ...
  do {
    ...
    for (const { address, pair: { token_0, token_1 } } of exchanges) {
      console.log(address)
      const poolData = await queryContract({ contract: address, data: { pair_info: {} } })
      console.log(poolData)
      return process.exit(0)
      pools.push({
        token0: token_0.contract_addr,
        token1: token_1.contract_addr
      })
    }
  } while (hasMore)
}
