...
async function getAllPairs(factory, chain) {
  ...
  let currentPairs;
  do {
    const queryStr = `{"pairs": { "limit": 30 ${
      allPairs.length
        ? `,"start_after":${JSON.stringify(
            allPairs[allPairs.length - 1].asset_infos
          )}`
        : ""
    } }}`;
    currentPairs = (
      await queryContract({ contract: factory, chain, data: queryStr })
    ).pairs;
    allPairs.push(...currentPairs);
  } while (currentPairs.length > 0);
  const dtos = [];
  const getPairPool = async (pair) => {
    const pairRes = await queryContract({
  ...
  await PromisePool.withConcurrency(25).for(allPairs).process(getPairPool);
  return dtos;
}

function getFactoryTvl(factory) {
  return async (_, _1, _2, { chain }) => {
    const pairs = (await getAllPairs(factory, chain)).filter(
      (pair) => pair.assets[0].balance && pair.assets[1].balance
    );
