...
const getTokenMetrics = async () => {
  ...
  const result = await fetch(subgraphUrl, {
    method: "POST",
    headers,
    body: JSON.stringify({ query: tokenMetricsQuery }),
  }).then((res) => res.json());
...
async function getData(isBorrowed = false) {
  ...
  return tokenMetrics.reduce((acc, token) => {
    ...
    const totalCollateralAmount = parseFloat(token.totalCollateralAmount || 0);
    acc[token.id] = isBorrowed ? totalBorrowedAmount : totalCollateralAmount + totalBorrowedAmount;
    return acc;
  }, {});
}
