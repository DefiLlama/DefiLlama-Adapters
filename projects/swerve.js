...
async function fetch() {
  var price_feed = await retry(
    async (bail) =>
      await axios.get(
        "https://api.coingecko.com/api/v3/simple/price?ids=bitcoin,cdai,compound-usd-coin&vs_currencies=usd&include_market_cap=true&include_24hr_vol=true&include_24hr_change=true"
      )
  );
...
async function calc(item, i, price_feed) {
  ...
  var poolAmount = await new BigNumber(balances)
    .div(10 ** coinDecimals[0][coins])
    .toFixed(2);
  ...
  if (item.type == "yToken") {
    var multiplier = 1;
    if (coins !== "0x8E870D67F660D95d5be530380D0eC0bd388289E1") {
      // PAX exception
