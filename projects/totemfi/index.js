...
async function tvl(timestamp, ethBlock, chainBlocks) {
  ...
  try {
    ...
    for (const graphUrl of graphUrls) {
      const { stakingPools } = await request(graphUrl, graphQuery, {
        block,
        timestamp,
      });
    ...
    for (let i = 0; i < allPrizePools.length; i++) {
      ...
      sdk.util.sumSingleBalance(
        balances,
        TOTM,
        BigNumber(allPrizePools[i]["totalStaked"])
          .minus(allPrizePools[i]["totalUnStaked"])
          .toFixed(0)
      );
    }
    return balances;
