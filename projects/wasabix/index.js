...
const vaults = [
  ...
  },
  {
    ...
    pools: [{ name: "vesper", address: contracts.vault.wbtc.vesper.address }],
  },
  {
    name: "lusd",
    token: tokens.lusd.address,
    pools: [{ name: "liquity", address: contracts.vault.lusd.liquity.address }],
  },
  {
    name: "weth",
    token: tokens.weth.address,
    pools: [{ name: "vesper", address: contracts.vault.weth.vesper.address }],
  },
];
...
const vaultsPolygon = [
  {
    ...
    pools: [
      {
        name: "polyquity",
        address: contractsPolygon.vault.pusd.polyquity.address,
      },
    ],
  },
];
...
async function eth(timestamp, block) {
  ...
  for (let vault of vaultsInfo) {
    ...
    sdk.util.sumSingleBalance(
      balances,
      _.find(vaultCalls, { target: poolAddr }).token,
      totalDeposited
    );
  }
...
async function bsc(timestamp, block, chainBlocks) {
  ...
  const wasabiPrice = await getWasabiPrice(
    "bsc",
    chainBlocks,
    prices.data.wbnb.usd
  );

  const lockerTvl =
    BigNumber(tokenStaked.output).div(Math.pow(10, 18)) * wasabiPrice;
  ...
  for (let pool of poolsInfo) {
    ...
    if (pid == 0) {
      ...
      poolTvl +=
        BigNumber(wbnbDeposited).div(Math.pow(10, 18)).toNumber() *
        prices.data.wbnb.usd *
        2;
    } else if (pid == 1) {
      poolTvl += BigNumber(totalDeposited).div(Math.pow(10, 18)).toNumber();
    } else if (pid == 2) {
      poolTvl += BigNumber(totalDeposited * 2)
        .div(Math.pow(10, 18))
        .toNumber();
    }
  }
...
async function polygon(timestamp, block, chainBlocks) {
  ...
  const lockerTvl =
    BigNumber(tokenStaked.output).div(Math.pow(10, 18)).toNumber() *
    wasabiPrice;
  // console.log('lockerTvl', lockerTvl)
