...
async function getHubAssets(api) {
  const assetCounts = await api.multiCall({ abi: abis.getAssetCount, calls: HUBS });
  const calls = HUBS.flatMap((hub, i) =>
...
module.exports = {
  methodology: "Counts the tokens held in Aave V4 Hub contracts.",
