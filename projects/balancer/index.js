...
async function ethereum(timestamp, ethBlock) {
  const [v1, v2] = await Promise.all([
    getTVL("balancer", ethBlock),
    getTVL("balancer-v2", ethBlock),
  ]);
...
async function polygon(timestamp, ethBlock, chainBlocks) {
  return toUSDTBalances(
    await getTVL(
      "balancer-polygon-v2",
      await getBlock(timestamp, "polygon", chainBlocks)
    )
  );
}
...
async function arbitrum(timestamp, ethBlock, chainBlocks) {
  return toUSDTBalances(
    await getTVL(
      "balancer-arbitrum-v2",
      await getBlock(timestamp, "arbitrum", chainBlocks)
    )
  );
}
