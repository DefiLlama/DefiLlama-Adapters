...
tvl = chain => async (timestamp, ethBlock, chainBlocks) => {
  ...
  const [{output: purchaseToken}, {output: rewardsAddress}] = await Promise.all([
    ...
    sdk.api.abi.multiCall({
      calls: poolsAddressesCalls,
      abi: abi["aelinPool_aelinRewardsAddress"],
      chain,
      block,
    })
  ]);
  const balancesCall = rewardsAddress.map((rewardsContract, i) => 
    ({ target: purchaseToken[i].output, params: [rewardsContract.output] }))
  console.log(balancesCall)
  const rewardsBalances = await sdk.api.abi.multiCall({
    calls: balancesCall,
      abi: "erc20:balanceOf",
      chain,
    block,
  ...
  const balances = {};
  sdk.util.sumMultiBalanceOf(balances, rewardsBalances, true, x => `${chain}:${x}`);
  return balances
}

stakingTVL = chain => async (timestamp, ethBlock, chainBlocks) => {
  if (chain === 'ethereum') {
    return {}
  } 

  const staked = await staking(
    aelin_data[chain]['AELIN_staking'], // stakingContract, 
    aelin_data[chain]['AELIN'],         // stakingToken, 
    chain
  ) (timestamp, ethBlock, chainBlocks)
  return staked
}

pool2TVL = chain => async (timestamp, ethBlock, chainBlocks) => {
...
module.exports = {
  ...
  methodology: 'Aelin TVL consists of tokens held by pools rewardContracts',
}
