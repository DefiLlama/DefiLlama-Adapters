...
const AVAX_STABILITY_POOL_ADDRESS =
  "0x4EBb1d6043e267EF5f2b88Ee18D414F541fD1b5C";
...
async function tvl(_, _ethBlock, chainBlocks) {
  ...
  const FarmPoolBalance_SUSD_USDC = BigNumber(LPValue_SUSD_USDC)
    .times(2)
    .times(FarmPoolLPBalance_SUSD_USDC)
    .div(LPTotalSupply_SUSD_USDC);
  ...
  const FarmPoolBalance_SET_USDC = BigNumber(LPValue_SET_USDC)
    .times(2)
    .times(FarmPoolLPBalance_SET_USDC)
    .div(LPTotalSupply_SET_USDC);

  const FarmPoolBalance = FarmPoolBalance_SUSD_USDC.plus(
    FarmPoolBalance_SET_USDC
  );
  ...
  const StabilityPoolTotalBalance = BigNumber(StabilityPoolBalanceOfAVAX)
    .plus(StabilityPoolBalanceOfETH)
    .plus(StabilityPoolBalanceOfBTC)
    .plus(StabilityPoolBalanceOfDAI)
    .toFixed();
