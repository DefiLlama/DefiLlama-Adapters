...
const getAddresses = gql`
  query TokenPairs($block: Int) {
    pools(first: 100, where: { optionType: CALL }) {
      name
      address
      base {
        address
...
function chainTvl(chain) {
  return async (time, _ethBlock, chainBlocks) => {
    ...
    const { pools } = await request(chainToSubgraph[chain], getAddresses, {
      block,
    });
    const balances = {};
    await sumTokens(
      balances,
      pools
        .map((p) => [p.underlying.address, p.address])
        .concat(pools.map((p) => [p.base.address, p.address])),
      block,
      chain,
      (addr) => `${chain}:${addr}`
    );
...
module.exports = {
  ...
  fantom: {
    tvl: chainTvl("fantom"),
    staking: stakings(
      ["0x9BCb8cE123E4bFA53C2319b12DbFB6F7B7675a30"],
  ...
  optimism: {
    ...
    staking: stakings(
      ["0x17BAe0E202f6A22f2631B037C0660A88990d6023"],
