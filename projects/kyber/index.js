...
async function ethTvl(timestamp, block) {
  const balances = {};
  const pairs = (await utils.fetchURL(`https://api.kyber.network/currencies`))
    .data.data;
  ...
  pairs.forEach((pair) => {
    if (pair.reserves_src) {
      const pairReserves = new Set(
        pair.reserves_src.concat(pair.reserves_dest)
      );
  ...
  await Promise.all(
    tokenWallets.map(async (result) => {
      if (!reserves.has(result.output)) {
        ...
        if ((await web3.eth.getCode(tokenWallet)) === "0x") {
          // Ignore if EOA
          return;
        }
  ...
  const ethBalance = BigNumber.sum(
    ...(await ethBalances).output.map((result) => result.balance)
  );
