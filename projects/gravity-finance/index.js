...
async function tvl(timestamp, block, chainBlocks) {
  ...
  let gfiUsdTokens = (
    await axios.get(
      `https://api.covalenthq.com/v1/137/address/${gfiUsdFarm}/balances_v2/?&key=ckey_72cd3b74b4a048c9bc671f7c5a6`
    )
  ).data.data.items;
  let gfiWethTokens = (
    await axios.get(
      `https://api.covalenthq.com/v1/137/address/${gfiWethFarm}/balances_v2/?&key=ckey_72cd3b74b4a048c9bc671f7c5a6`
    )
  ).data.data.items;
  let gfiTokens = (
    await axios.get(
      `https://api.covalenthq.com/v1/137/address/${gfiFarm}/balances_v2/?&key=ckey_72cd3b74b4a048c9bc671f7c5a6`
    )
  ).data.data.items;
  let chainLinkTokens = (
    await axios.get(
      `https://api.covalenthq.com/v1/137/address/${chainLinkFarm}/balances_v2/?&key=ckey_72cd3b74b4a048c9bc671f7c5a6`
    )
  ).data.data.items;
  let sushiTokens = (
    await axios.get(
      `https://api.covalenthq.com/v1/137/address/${sushiFarm}/balances_v2/?&key=ckey_72cd3b74b4a048c9bc671f7c5a6`
    )
  ).data.data.items;
    ...
    gfiUsdTokens.map(async (token) => {
      if (token.contract_ticker_symbol === "UNI-V2") {
        ...
        sdk.util.sumSingleBalance(
          balances,
          transformAddress(token.contract_address),
