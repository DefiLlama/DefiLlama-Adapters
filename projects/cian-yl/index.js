...
const { getLogs } = require("@defillama/sdk/build/util/indexer");
...
const eventAbis = {
  confirmWithdrawal: "event ConfirmWithdrawal(address[] users, uint256[] amounts)"
}

const tvl = async (api) => {
  const toBlock = await api.getBlock() - 100
  const { rawVaults, preVaults } = config[api.chain]
  const vaults = [...new Set(rawVaults.map(i => i.toLowerCase()))]
  const vaultParams = await api.multiCall({ abi: abis.getVaultParams, calls: preVaults })
  const redeemOperators = vaultParams.map(({ redeemOperator }) => redeemOperator)

  const tests = await getLogs({ target: redeemOperators[0], eventAbi: eventAbis.confirmWithdrawal, fromBlock: 21536253, toBlock, onlyArgs: true })
  let total = 0n;

  tests.forEach(([_users, amounts]) => {
    if (Array.isArray(amounts)) {
      const sum = amounts.reduce((acc, val) => acc + val, 0n);
      total += sum;
    }
  });
