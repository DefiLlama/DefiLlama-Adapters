...
const openPositions = require('./openPositions.json')
...
async function updatePositionsList() {
  ...
  const positions = [...Array(+next_position_id).keys()]
    .map((_value, position) => (`{ chain_id: 3, position_id: "${position}" }`)).slice(currentQueriedCount)

  const chunkSize = 1
  for (let i = 0; i < positions.length; i += chunkSize) {
    if (i % 100)
      require('fs').writeFileSync('openPositions.json', JSON.stringify(openPositions))
