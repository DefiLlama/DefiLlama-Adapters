...
function tvl(time) {
  return getApiTvl(time, current, async () => {
    const dayData = await utils.fetchURL(
      "https://sodaki.com/api/historical-tvl"
    );
...
module.exports = {
  methodology:
    'TVL counts the tokens locked in the Ref Finance liquidity pools, data is pulled from the Sodaki API:"https://sodaki.com/api/last-tvl".',
  tvl,
};
