...
async function getTokens(api, owners, isVesting) {
  let tokens = (
    await Promise.all(
      owners.map((i) => covalentGetTokens(i, api, { onlyWhitelisted: false }))
    )
  )
    .flat()
    .filter((i) => !blacklistedTokens.includes(i))
    .filter((i) => i !== ADDRESSES.null);
...
module.exports = {
  hallmarks: [
    [
      Math.floor(new Date("2022-10-03") / 1e3),
      "Vesting tokens are not included in tvl",
    ],
  ],
  start: 1573582731,
  timetravel: false,
  ronin: {
    tvl: sumTokensExport({
      ...
      tokens: [ADDRESSES.ronin.USDC, ADDRESSES.ronin.WETH, ADDRESSES.ronin.AXS],
    }),
  },
};
