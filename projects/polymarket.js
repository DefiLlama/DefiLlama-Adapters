...
const graphQuery = gql`
  query GET_POLYMARKET($skip: Int, $block: Int) {
    conditions(first: 1000, skip: $skip, block: { number: $block }) {
      id
      oracle
      questionId
      outcomeSlotCount
      resolutionTimestamp
      fixedProductMarketMakers(orderBy: creationTimestamp) {
        id
        creationTimestamp
        collateralToken {
          id
...
async function getMarketsLiquidity_graphql(timestamp, block, chainBlocks) {
  ...
  while (skip !== -1) {
    const { conditions } = await request(graphUrl, graphQuery, { skip, block });
    skip += 1000;
    console.log(
      `${conditions && conditions.length} conditions found for skip: ${skip}`
    );
    if (conditions && conditions.length > 0) {
      conditions.forEach((condition) => {
        condition.fixedProductMarketMakers.forEach((fpmm) => {
          console.log(fpmm);
          scaledLiquidityParameterSum = scaledLiquidityParameterSum.plus(
            BigNumber(fpmm.scaledLiquidityParameter)
          );
        });
      });
    } else {
...
const polymarket_api_url =
  "https://strapi-matic.poly.market/markets?_limit=-1&_sort=volume:desc"; // &active=true
async function getMarketsLiquidity_api() {
  let markets = await utils.fetchURL(polymarket_api_url);
  // Filter out unwanted fields
  markets = markets.data.map(
  ...
  const marketsLiquidity = markets.reduce(
    (acc, market) => acc.plus(BigNumber(market.liquidity)),
    BigNumber(0)
  );
