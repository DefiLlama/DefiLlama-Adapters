...
async function tvl(timestamp) {
  ...
  await Promise.all(
    Object.keys(pairs).map(async (pair) => {
      const { assets } = (
        await axios.get(
          `https://lcd.terra.dev/wasm/contracts/${pair}/store?query_msg={%22pool%22:{}}` /*&height=${block - (block % 100)}`*/
        )
      ).data.result; // Node is semi-pruned, only every 100th block is stored
