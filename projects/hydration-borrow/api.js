...
const TOKEN_MAP = {
  ...
  "0x00000000000000000000000000000001000f453d": { symbol: "tBTC", decimals: 18 },
  "0x00000000000000000000000000000001000002b2": { symbol: "GDOT", decimals: 18 },
}
...
const cgMapping = {
  ...
  iBTC: 'interbtc',
  GDOT: 'gigadot', // Updated GDOT CoinGecko ID
}
...
async function calculateTotalSupplied(api) { // Renamed from calculateAvailableLiquidity
  ...
  for (const reserve of reservesData) {
    ...
    const { cgId, decimals, availableLiquidity, totalBorrowed } = formatReserveData(reserve)
    const totalSuppliedForReserve = BigInt(availableLiquidity) + BigInt(totalBorrowed) // Sum available and borrowed
    const normalized = Number(totalSuppliedForReserve) / (10 ** decimals)
    api.add(cgId, normalized, { skipChain: true })
  }
  return api.getBalances()
}

async function calculateTotalBorrowed(api) {
  const contract = new ethers.Contract(UI_POOL_DATA_PROVIDER, [abi.getReservesData], provider)
  const [reservesData] = await contract.getReservesData(POOL_ADDRESS_PROVIDER)

  for (const reserve of reservesData) {
    if (!reserve.isActive || reserve.isFrozen) continue
    const { cgId, totalBorrowed, decimals } = formatReserveData(reserve)
    const normalized = Number(BigInt(totalBorrowed)) / (10 ** decimals)
    api.add(cgId, normalized, { skipChain: true })
  }
  return api.getBalances()
}
...
module.exports = {
  tvl: calculateTotalSupplied,      // TVL is now total supplied
  borrowed: calculateTotalBorrowed,
}
