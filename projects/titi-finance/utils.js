...
function stakingUnknownPricedLPInSyncSwap(stakingContract, stakingToken, chain, lpContract, coingeckoIdOfPairedToken, decimals) {
    transform = ()=>coingeckoIdOfPairedToken;
...
function pool2sEthereum(stakingContracts, lpTokens, titiPair, chain = "ethereum", transformAddress = undefined) {
    return async (_timestamp, _ethBlock, _, { api }) =>  {
        ...
        titiBal = balances['ethereum:0x3bdffa70f4b4e6985eed50453c7c0d4a15dcec52'];
        tiusdBal = balances['ethereum:0x6eff556748ee452cbdaf31bcb8c76a28651509bd'];

        titiValue = BigNumber(titiBal).times(titiReserveAmounts[1]).times(BigNumber(10**12)).div(titiReserveAmounts[0]);
        tiusdValue = BigNumber(tiusdBal);
...
function pool2sEra(stakingContracts, lpTokens, titiPair, chain = "era", transformAddress = undefined) {
    return async (_timestamp, _ethBlock, _, { api }) =>  {
        ...
        titiBal = balances['era:0x4ebfb78c4780c304dff7de518db630b67e3f044b'];
        tiusdBal = balances['era:0xc059767cb62f003e863f9e7bd1fc813beff9693c'];

        titiValue = BigNumber(titiBal).times(titiReserveAmounts[0]).times(BigNumber(10**12)).div(titiReserveAmounts[1]);
        tiusdValue = BigNumber(tiusdBal);
