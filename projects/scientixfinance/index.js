...
async function getBUSDLpPrice(lpTokenAddress, BUSDAddress, scUSDAddress) {
  try {
    ...
    return new BigNumber(BUSDBalance)
      .plus(new BigNumber(scUSDBalance))
...
async function getAalpacaTotal(vault) {
  ...
  return alpacaContract.methods.totalToken().call();
}

async function getAalpacaTotalSupply(vault) {
  const alpacaContract = new web3.eth.Contract(alpacaVaultAbi, vault);
  return alpacaContract.methods.totalSupply().call();
}
...
async function getIbAlpacaPrice(vault) {
  try {
    ...
    return new BigNumber(price)
      .times(new BigNumber(totalToken))
      .div(new BigNumber(totalSupply));
  } catch (e) {
...
async function getFarmBalance() {
  ...
  // price
  const BusdScixLPPrice = new BigNumber(BUSDBalance)
    .times(2)
    .div(new BigNumber(BusdScixLpSupply));
  const scixPrice = new BigNumber(BUSDBalance).dividedBy(
    new BigNumber(SCIXBalance)
  );
...
function getGWeiFromWei(wei, decimals = 18) {
  return BigNumber(wei).dividedBy(BigNumber(10).pow(decimals)).toFixed(2);
}
...
async function fetch() {
  ...
  // transmute
  tvl = tvl
    .plus(new BigNumber(stakedScUsd))
    .plus(new BigNumber(transmuteBUSD))
    .plus(new BigNumber(transmuteAdapterBUSD));
  // farm
