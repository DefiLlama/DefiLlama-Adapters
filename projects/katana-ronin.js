...
const token_lookup_table = {
  // needed to add 0x in front
...
const factoryQuery = gql`
  query get_tvl($block: Int) {
    katanaFactories(block: { number: $block }) {
      id
      totalLiquidity
    }
  }
...
async function tvl(timestamp, ethBlock, chainBlocks) {
  ...
  const { tokens } = await request(graphUrl, tokensQuery, { block });
  ...
  tokens.forEach((token) => {
    sdk.util.sumSingleBalance(
      balances,
      transform(token.id),
      BigNumber(token.totalLiquidity)
