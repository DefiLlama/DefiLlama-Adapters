...
const calcTvl = async (balances, block, token, balance) => {
  ...
  if (balance == erc20.balanceOf) {
    ...
    for (let index = 0; index < pairs.length; index++) {
      sdk.util.sumSingleBalance(balances, tokens[index], balancePairs[index]);
    }
  } else {
    const totalDebt = (
      await sdk.api.abi.multiCall({
    ...
    for (let index = 0; index < pairs.length; index++) {
      sdk.util.sumSingleBalance(balances, tokens[index], totalDebt[index]);
    }
  }
};
...
const ethTvl = async (timestamp, ethBlock, chainBlocks) => {
  ...
  await calcTvl(balances, ethBlock, abi.tokenA, erc20.balanceOf);

  await calcTvl(balances, ethBlock, abi.tokenB, erc20.balanceOf);

  //  --- Total of TVL's pairs with using ---
  await calcTvl(balances, ethBlock, abi.tokenA, abi.totalDebt);

  await calcTvl(balances, ethBlock, abi.tokenB, abi.totalDebt);
