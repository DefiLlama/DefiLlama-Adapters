...
async function tvl(timestamp, ethBlock) {
  const addresses = (await utils.fetchURL(addressAPI)).data.map(
    (t) => t.fragolaAddress
  );
  const balances = {};
  const [
    ...
    ["token0", "token1", "token0PerShareStored", "token1PerShareStored"]
      .map((method) => abi[method])
      .concat(["erc20:totalSupply"])
      .map((abi) =>
  ...
  for (let i = 0; i < addresses.length; i++) {
    sdk.util.sumSingleBalance(
      ...
      BigNumber(token0PerShareStored.output[i].output)
        .times(totalSupply.output[i].output)
        .div(1e18)
        .toFixed(0)
    );
    sdk.util.sumSingleBalance(
      ...
      BigNumber(token1PerShareStored.output[i].output)
        .times(totalSupply.output[i].output)
        .div(1e18)
        .toFixed(0)
    );
