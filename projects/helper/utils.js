...
function isLP(symbol, token, chain) {
  ...
  if (chain === "scroll" && /(cSLP|sSLP|ZFLP)$/.test(symbol)) return true; // syncswap LP
  ...
  if (isLPRes && !["UNI-V2", "Cake-LP"].includes(symbol)) sdk.log(chain, symbol, token);
...
async function getBalance(chain, account) {
  switch (chain) {
    ...
    case "bep2":
      const balObject = (await http.get(`https://api-binance-mainnet.cosmostation.io/v1/account/${account}`)).balances.find(
        (i) => i.symbol === "BNB"
      );
