  ...
  return async (_, ethBlock, chainBlocks) => {
    ...
    COLLATERAL_ADDRESSES.forEach(async (collateral, index) => {
      ...
      if (collateralKeys[0].includes('lps')) {
        //if lp tokens
        const tokenA = await getResponse(collateral[collateralKeys[0]][1], token0Abi, block, chain)
        const tokenB = await getResponse(collateral[collateralKeys[0]][1], token1Abi, block, chain)
        const lpTokenAmounts = await getResponse(collateral[collateralKeys[0]][1], getReserveAbi, block, chain)
        const totalSupplyLP = await getResponse(collateral[collateralKeys[0]][1], totalSupplyAbi, block, chain)
        const pid = await getResponse(collateral[collateralKeys[0]][0], pidAbi, block, chain)
        const balanceOf = await getResponse(collateral[collateralKeys[0]][2], userInfoAbi, block, chain, [pid.output, collateral[collateralKeys[0]][0]])
