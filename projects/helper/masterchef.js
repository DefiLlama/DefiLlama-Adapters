...
function isLP(symbol) {
  return (
    symbol.includes("LP") || symbol.includes("PGL") || symbol.includes("UNI-V2")
  );
}
...
async function addFundsInMasterChef(
  ...
  if (excludePool2) {
    ...
    [token0, token1, balance].forEach((_, idx) => {
      if (
        lpTokens[idx] === undefined ||
        token0.output[idx].output.toLowerCase() ===
          stakingToken.toLowerCase() ||
        token1.output[idx].output.toLowerCase() === stakingToken.toLowerCase()
      ) {
        return;
      }
      lpPositions.push({
