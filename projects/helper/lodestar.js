...
async function getMarkets(comptroller, block, chain, cether, cetheEquivalent, blacklist = [], abis={}) {
  ...
  async function _getMarkets() {
    ...
    underlyings.output.forEach(({ output, input: { target } }) => 
        {
            if (target.toLowerCase() === '0xb4d58c1f5870efa4b05519a72851227f05743273'.toLowerCase()) {
                markets.push({ cToken: target, underlying: '0x0000000000000000000000000000000000000000' })
            } else {
                markets.push({ cToken: target, underlying: output })
            }
        }
    )
    return markets;
