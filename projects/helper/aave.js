...
function aaveChainTvl(
	...
	borrowed,
	v3 = false,
	{ abis = {}, oracle, blacklistedTokens = [], hasV2LPs = false } = {}
) {
	return async (api) => {
		const chain = api.chain
		const block = api.block
		const balances = {}
		const { transformAddress, fixBalances, v2Atokens, v2ReserveTokens, dataHelper, updateBalances } = await getData({
			oracle,
			chain,
			block,
			addressesProviderRegistry,
...
function aaveExports(
	_chain,
	addressesProviderRegistry,
	transform = undefined,
	dataHelpers = undefined,
	{ oracle, abis, v3 = false, blacklistedTokens = [], hasV2LPs = false } = {}
) {
	return {
		tvl: aaveChainTvl(_chain, addressesProviderRegistry, transform, dataHelpers, false, v3, {
			oracle,
			abis,
			blacklistedTokens,
			hasV2LPs,
		}),
		borrowed: aaveChainTvl(_chain, addressesProviderRegistry, transform, dataHelpers, true, v3, {
			oracle,
			abis,
			hasV2LPs,
			blacklistedTokens,
...
function aaveV2Export(
	registry,
	{ useOracle = false, baseCurrency, baseCurrencyUnit, abis = {}, fromBlock, blacklistedTokens = [] } = {}
) {
	async function tvl(api) {
		const data = await getReservesData(api)
		const tokensAndOwners = data.map((i) => [i.underlying, i.aTokenAddress])
		if (!useOracle) return sumTokens2({ tokensAndOwners, api, blacklistedTokens })
		const balances = {}
	...
	async function getReservesData(api) {
		...
		const data = await api.multiCall({
			abi: abis.getReserveData ?? abiv2.getReserveData,
			calls: tokens,
			target: registry,
		})
	...
	async function getReservesDataFromBlock(api) {
		const logs = await getLogs({
			...
			fromBlock,
			eventAbi:
				'event ReserveInitialized (address indexed underlying, address indexed aTokenAddress, address stableDebtTokenAddress, address variableDebtTokenAddress, address interestRateStrategyAddress)',
			onlyArgs: true,
		})
		return logs
	}
