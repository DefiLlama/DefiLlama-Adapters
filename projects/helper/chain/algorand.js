...
async function sumTokens({
    ...
    if (tinymanLps.length) {
        await Promise.all(
            tinymanLps.map(([lp, unknown]) =>
                resolveTinymanLp({
                    balances,
                    lpId: lp,
                    unknownAsset: unknown,
                    blacklistedTokens: blacklistOnLpAsWell
                        ? blacklistedTokens
                        : []
                })
            )
    ...
    if (lpBalance && lpBalance !== "0") {
        ...
        if (unknownAsset && lpInfo.assets[unknownAsset]) {
            ...
            Object.keys(lpInfo.assets).forEach((token) => {
                if (
                    !blacklistedTokens.length ||
                    !blacklistedTokens.includes(token)
                )
                    if (token !== unknownAsset)
                        sdk.util.sumSingleBalance(
                            ...
                            token,
                            BigNumber(
                                lpInfo.assets[token].amount * ratio
                            ).toFixed(0),
                            "algorand"
                        )
            })
            ...
            Object.keys(lpInfo.assets).forEach((token) => {
                if (
                    ...
                    sdk.util.sumSingleBalance(
                        balances,
                        token,
                        BigNumber(lpInfo.assets[token].amount * ratio).toFixed(
                            0
                        ),
                        "algorand"
                    )
