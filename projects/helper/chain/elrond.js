...
const call = async ({ target, abi, params = [], responseTypes = [] }) => {
  const data = await post(getEnv("MULTIVERSX_RPC") + "/query", {
    scAddress: target,
    funcName: abi,
    args: params,
  });
  ...
  function parseResponses(item, idx) {
    ...
    switch (responseTypes[idx]) {
      ...
      default:
        throw new Error("Unknown/unsupported data type");
    }
  }
...
async function getElrondBalance(address) {
  ...
  } = await get(`${getEnv("MULTIVERSX_RPC")}/address/${address}`);
  return balance;
}
...
async function getTokens({
  ...
  const res = await get(
    `${getEnv("MULTIVERSX_RPC")}/accounts/${address}/tokens?size=1000`
  );
  res
    .filter((i) => i.type === "FungibleESDT")
    .forEach((i) => {
      const token = i.identifier;
      if (tokens.length && !tokens.includes(token)) return; // sum only whitelistedTokens
      if (whitelistedTokens.length && !whitelistedTokens.includes(token))
        return; // sum only whitelistedTokens
...
async function sumTokens({
  ...
  if (tokensAndOwners.length) {
    await Promise.all(
      tokensAndOwners.map(([token, owner]) =>
        sumTokens({
          owners: [owner],
          tokens: [token],
          balances,
  ...
  if (
    (!tokens.length || tokens.includes(nullAddress)) &&
    (!whitelistedTokens.length || whitelistedTokens.includes(nullAddress)) &&
    (!blacklistedTokens.length || !blacklistedTokens.includes(nullAddress))
  ) {
    const { errors } = await PromisePool.withConcurrency(5)
      .for(owners)
      .process(async (i) => {
...
async function getNFTs(address) {
  const res = await get(
    `${getEnv("MULTIVERSX_RPC")}/accounts/${address}/nfts?size=1000`
  );
  return res;
}
