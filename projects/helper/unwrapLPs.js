...
async function unwrapYearn(
  ...
  sdk.util.sumSingleBalance(
    balances,
    transformAddress(underlying),
    (balances[yToken] * pricePerShare.output) /
      10 ** (await sdk.api.erc20.decimals(underlying, chain)).output
  );
...
async function unwrapCrv(
  ...
  if (
    crvToken.toLowerCase() === "0x06325440d014e39736583c165c2963ba99faf14e" ||
    crvToken.toLowerCase() === "0xa3d87fffce63b53e0d54faa1cc983b7eb0b74a9c"
  ) {
  ...
  underlyingSwapTokens.forEach((call) => {
    ...
    const underlyingBalance = BigNumber(call.output)
      .times(balance3Crv)
