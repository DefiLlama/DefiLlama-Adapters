 ...
 *      For example, on Rootstock (which implemented EIP-1191) any js/ts call to the blockchain for getting/connecting to a contract
...
function formatAddressChecksum(address, netwrokName) {
  ...
  return ["rootstock"].includes(netwrokName?.toLowerCase())
    ? address.toLowerCase()
    : address;
}
