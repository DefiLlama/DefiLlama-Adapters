...
async function calculateUniTvl(getAddress, block, chain, FACTORY, START_BLOCK, useMulticall = false, abis={
  ...
  if (useMulticall) {
    ...
    const chunkSize = 100;
    for (let i = 0; i < pairNums.length; i += chunkSize) {
      const pairsChunk = pairNums.slice(i, i + chunkSize);
      const pairsChunkResponse = (await sdk.api.abi.multiCall({
        abi: factoryAbi.allPairs,
        chain,
        calls: pairsChunk.map(num => ({
          target: FACTORY,
          params: [num]
        })),
        block,
  ...
  const balances = {}
  const addressesChunkSize = 100;
  for (let i = 0; i < pairAddresses.length; i += addressesChunkSize)
    await addToBalances(balances, pairAddresses.slice(i, i + addressesChunkSize))

  return balances

  async function addToBalances(balances, pairAddresses) {
