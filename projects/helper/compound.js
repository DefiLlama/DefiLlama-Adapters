...
async function getMarkets(comptroller, block, chain, cether, cetheEquivalent) {
  ...
  await Promise.all(
    allCTokens.map(async (cToken) => {
      try {
        let underlying = await getUnderlying(
          block,
          chain,
          cToken,
          cether,
...
function getCompoundV2Tvl(
  comptroller,
  chain = "ethereum",
  transformAdress = (addr) => addr,
  cether = "0x4Ddc2D193948926D02f9B1fE9e1daa0718270ED5",
  cetheEquivalent = "0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2",
  borrowed = false,
  checkForLPTokens = undefined
) {
  return async (timestamp, ethBlock, chainBlocks) => {
    ...
    _.each(markets, (market, idx) => {
      ...
      if (
        checkForLPTokens !== undefined &&
        checkForLPTokens(symbols.output[idx].output)
      ) {
        lpPositions.push({
        ...
        sdk.util.sumSingleBalance(
          balances,
          transformAdress(market.underlying),
          getCash.output
        );
      }
    });
    ...
    await Promise.all(
      allMarkets.map(async (token) => {
        let amount = new BigNumber(
          await getCash(block, chain, token, borrowed)
        );
        ...
        let underlyingPrice = new BigNumber(
          await getUnderlyingPrice(
            block,
            chain,
            oracle,
            token,
            abis.underlyingPrice
          )
...
function compoundExportsWithAsyncTransform(
  comptroller,
  chain,
  cether,
  cetheEquivalent,
  transformAdressConstructor
) {
  return {
    tvl: async (...args) => {
