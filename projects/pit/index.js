...
async function tvl(api) {
  ...
  const allTokens = [...directionalTokens, ...directionalATokens].filter(Boolean)
  await api.sumTokens({ tokensAndOwners2: [allTokens, Array(allTokens.length).fill().map((_, i) => directionalVaults[i % directionalVaults.length])] })
