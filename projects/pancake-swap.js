...
async function fetch() {
  let response = await utils.fetchURL(
...
async function tvl(timestamp, ethBlock, chainBlocks) {
  if (Math.abs(timestamp - Date.now() / 1000) < 3600) {
    ...
    if (Math.abs(closest.date - timestamp) > 3600 * 24) {
      // Oldest data is too recent
      const { uniswapFactories } = await request(graphUrl, graphQuery, {
...
module.exports = {
  ...
  methodology:
    "TVL accounts for the liquidity on all AMM pools, using the TVL chart on https://pancakeswap.info/ as the source. Staking accounts for the CAKE locked in MasterChef (0x73feaa1eE314F8c655E354234017bE2193C9E24E)",
  staking: {
    tvl: staking,
  },
  tvl,
