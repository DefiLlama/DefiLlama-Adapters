...
Object.keys(chainConfig).forEach(chain => {
  ...
  async function tvl(ts, _block, chainBlocks) {
    ...
    for (const c of results.output)
      sdk.util.sumSingleBalance(balances, chainData.coin, c.balance)

    // Scale balances to correct decimals
    balances[chainData.coin] = BigNumber(balances[chainData.coin] || 0).dividedBy(BigNumber(10).pow(18));
