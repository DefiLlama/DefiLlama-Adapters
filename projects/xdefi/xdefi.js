...
async function fetch() {
  ...
  var xhalflifeApi =
    "https://api.thegraph.com/subgraphs/name/xdefilab/xhalflife";
  ...
  `;
  const { streamTotalDatas } = await xhalflifeGraphQLClient.request(
    xhalflifeQuery
  );
  ...
  const tokenPriceQuery = gql`
    query getTokenPrices($ids: [String!]) {
      tokenPrices(first: 100, where: { id_in: $ids }) {
        id
        symbol
        price
      }
  ...
  const { tokenPrices } = await request(xdexApi, tokenPriceQuery, {
    ids: Object.keys(tokenDataWithLocked).concat(wethAddress),
  });
  ...
  Object.keys(tokenDataWithLocked).forEach((token) => {
    ...
    if (findToken) {
      ...
      if (token === ethAddress) {
        tokenDataWithLocked[token].price = tokenPrices.find(
          (tokenPrice) => tokenPrice.id == wethAddress
        ).price;
      } else {
  ...
  const tokenTotalValue = Object.keys(tokenDataWithLocked).reduce(
    (all, current) => {
      return all.plus(
        new BigNumber(tokenDataWithLocked[current].price).times(
          tokenDataWithLocked[current].locked
        )
      );
    },
    new BigNumber(0)
