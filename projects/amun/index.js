...
async function getSolanaTokenRate(address) {
    return await axios.get("https://lima-api.amun.com/tokenRate/APygy51vntbQQsYmgkF1CeMsAn5Tv1rsVUB6nee3wJpY?chain=solana&address="+address);
}
...
async function fetch(timestamp, ethBlock, chainBlocks) {
	const ethereumTVL = await chainTvl("ethereum", [DFI_ADDRESS])(timestamp, ethBlock, chainBlocks);
	const polygonTVL = await chainTvl("polygon", [PECO_ADDRESS])(timestamp, ethBlock, chainBlocks);
	const solanaTVL = await chainTvl("solana", [SOLI_ADDRESS])(timestamp, ethBlock, chainBlocks);
