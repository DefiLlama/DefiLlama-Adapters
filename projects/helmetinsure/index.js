...
const THEGARPH_API = {
  [CHAIN_POLYGON]:
    "https://api.thegraph.com/subgraphs/name/app-helmet-insure/guard",
  [CHAIN_BSC]:
...
function fetch(chain) {
  return async (_timestamp, _ethBlock, chainBlocks) => {
    ...
    var query = gql`
      query tvl($block: Int) {
        options(first: 1000, block: { number: $block }) {
          collateral
          asks {
            volume
            settleToken
    ...
    const data = options
      ...
      .reduce((_data, ask) => {
        ...
        if (typeof _data[key] === "undefined") {
          ...
          _data[key] = new BigNumber(_data[key])
            .plus(new BigNumber(ask.volume))
            .toFixed();
        }
        return _data;
      }, {});
