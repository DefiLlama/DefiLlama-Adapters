...
const getStakeTvl = async (onxPrice) => {
  const balance = new BigNumber(
    await onxTokenContract.methods.balanceOf(tokenAddresses.sOnx).call()
  );
    ...
    farms.map(async (farm) => {
      ...
      if (!farm.isLpToken) {
        balance = new BigNumber(
          await farmContracts[farm.title].methods
            .balanceOf(tokenAddresses.onxFarm)
            .call()
        );
        switch (farm.title) {
          case "aEth":
        ...
        const totalSupply = new BigNumber(
          await farmContracts[farm.title].methods.totalSupply().call()
        );
        const { reserve0, reserve1 } = await getReserves(
          farmContracts[farm.title]
        );
        let sum;
        switch (farm.title) {
          case "onxEthLp":
            sum = new BigNumber(reserve0)
              .times(price.wethPrice)
              .plus(new BigNumber(reserve1).times(price.onxPrice))
              .div(1e18);
            break;
          case "onxEthSlp":
            sum = new BigNumber(reserve0)
          ...
          case "onxEthSlpMulti":
            balance = new BigNumber(
              await farmContracts[farm.title].methods
                .balanceOf(tokenAddresses.onxTripleFarm)
                .call()
            );
            sum = new BigNumber(reserve0)
              .times(price.wethPrice)
      ...
      switch (farm.title) {
        case "aeth":
          tokenBalance = new BigNumber(
            await aethTokenContract.methods.balanceOf(farm.address).call()
          );
          balance = tokenBalance.times(price.aethPrice);
          break;
        case "weth":
          tokenBalance = new BigNumber(
            await wethTokenContract.methods.balanceOf(farm.address).call()
          );
          balance = tokenBalance.times(price.wethPrice);
          break;
        case "onx":
          tokenBalance = new BigNumber(
            await onxTokenContract.methods.balanceOf(farm.address).call()
          );
          balance = tokenBalance.times(price.onxPrice);
          break;
        case "dai":
          tokenBalance = new BigNumber(
            await daiTokenContract.methods.balanceOf(farm.address).call()
          );
          balance = tokenBalance.times(1);
          break;
        case "frax":
          tokenBalance = new BigNumber(
            await fraxTokenContract.methods.balanceOf(farm.address).call()
          );
          balance = tokenBalance.times(1);
          break;
        case "usdc":
          tokenBalance = new BigNumber(
            await usdcTokenContract.methods.balanceOf(farm.address).call()
          );
...
const getOnsPoolsTvl = async (price) => {
  ...
  onsPools = await Promise.all(
    onsPools.map(async (farm) => {
      ...
      const totalSupply = new BigNumber(
        await onsPoolsContracts[farm.title].methods.totalSupply().call()
      );
      const tokenBalance = new BigNumber(
        await onsPoolsContracts[farm.title].methods
          .balanceOf(farm.address)
          .call()
      );
      const { reserve0, reserve1 } = await getReserves(
        onsPoolsContracts[farm.title]
      );
      if (farm.title === "aethPairOne") {
        sum = new BigNumber(reserve0)
          .times(price.aethPrice)
          .plus(new BigNumber(reserve1).times(price.onePrice));
      } else if (farm.title === "aethPairOns") {
        sum = new BigNumber(reserve0)
          .times(price.aethPrice)
          .plus(new BigNumber(reserve1).times(price.onsPrice));
      } else if (farm.title === "aethPairEth") {
        sum = new BigNumber(reserve0)
          .times(price.wethPrice)
          .plus(new BigNumber(reserve1).times(price.aethPrice));
      }
...
async function fetch() {
  ...
  const farmsTvl = await getFarmsTvl({
    wethPrice,
    onxPrice,
    aethPrice,
    ankrPrice,
  ...
  const onsPoolsTvl = await getOnsPoolsTvl({
    aethPrice,
    onePrice,
    onsPrice,
    wethPrice,
