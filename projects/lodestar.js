...
async function getMarkets(comptroller, block, chain, cether, cetheEquivalent, blacklist = [], abis={}) {
  ...
  async function _getMarkets() {
    ...
    allCTokens.forEach(cToken => {
      ...
      if (cToken === '0xb4d58C1F5870eFA4B05519A72851227F05743273'.toLowerCase()) {
        markets.push({ underlying: '0x0', cToken })  //cETH => WETH
        return;
      }
    ...
    const isLeth = underlyings.output.find(i => !i.output)
    if (isLeth)
      throw new Error(`${isLeth.input.target} market rugged, is that market LETH?`)
...
function getLodestarTvl(comptroller, chain = "arbitrum", transformAdress,
  cether = "0xb4d58C1F5870eFA4B05519A72851227F05743273", cetheEquivalent = "0x0",
...
function compoundExports(comptroller, chain, cether, cetheEquivalent, transformAdressRaw, checkForLPTokens, { blacklistedTokens = [] } = {}) {
  ...
  return {
    tvl: getLodestarTvl(comptroller, chain, transformAdressRaw, cether, cetheEquivalent, false, checkForLPTokens, { blacklistedTokens }),
    borrowed: getLodestarTvl(comptroller, chain, transformAdressRaw, cether, cetheEquivalent, true, checkForLPTokens, { blacklistedTokens })
  }
}
...
function compoundExportsWithAsyncTransform(comptroller, chain, cether, cetheEquivalent, transformAdressConstructor) {
  return {
    tvl: async (...args) => {
      ...
      return getLodestarTvl(comptroller, chain, transformAddress, cether, cetheEquivalent)(...args)
    },
    borrowed: async (...args) => {
      const transformAddress = await getChainTransform(chain)
      return getLodestarTvl(comptroller, chain, transformAddress, cether, cetheEquivalent, true)(...args)
    },
  }
}
