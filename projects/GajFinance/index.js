...
async function stakingPolygon(timestamp, ethBlock, chainBlocks) {
  ...
  await sumTokensAndLPsSharedOwners(
    balances,
    [[GAJ_TOKEN, false]],
    [NFTFARM_GAJ, MASTER_GAJ, JUNGLEPOOL],
    chainBlocks.polygon,
    "polygon",
    (addr) => `polygon:${addr}`
  );
...
function vaults(pool2, selectedChain) {
  return async (timestamp, ethBlock, chainBlocks) => {
    const balances = {};
    const vaults = (await fetchURL(pool2 ? nativeEndpoint : nonNativeEndpoint))
      .data;
    for (const vault of vaults) {
      // Can't aggregate calls because there are multiple chains
...
module.exports = {
  methodology:
    "TVL comes from NFT Farming, Jungle Pools, MasterChef and Vaults",
  tvl: vaults(false),
  avalanche: {
