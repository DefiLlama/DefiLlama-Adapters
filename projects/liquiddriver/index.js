...
const spiritLinspiritLpInBeethoven =
    ...
    if (symbols.output[idx].success) {
      if (symbols.output[idx].output.includes("LP")) {
        ...
        if (symbols.output[idx].output === "3poolV2-f") {
          ...
          const linSpiritInQuoteToken = new BigNumber(
            Number(reserves.output[0])
          )
            .div(Number(reserves.output[1]))
            .times(Number(lpBalances.output["1"][1]));
          beethovenTvlInSpirit = new BigNumber(
            Number(lpBalances.output["1"][0])
          )
            .plus(linSpiritInQuoteToken)
            .times(lpTokenRatio)
            .toFixed(0);
        } else {
...
module.exports = {
  fantom: {
    ...
    tvl: sdk.util.sumChainTvls([masterchefTvl, minichefTvl]),
  },
};
