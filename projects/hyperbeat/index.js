...
module.exports = {
  ...
  hyperliquid: {
    tvl: async (api) => {
      ...
      const _morphoVaults = Object.keys(morphoVaults)

      const [supplies, vaultDecimals] = await Promise.all([
        api.multiCall({ calls: _morphoVaults, abi: 'erc20:totalSupply', }),
        api.multiCall({ calls: _morphoVaults, abi: 'erc20:decimals', })
      ]);

      supplies.forEach((data, i) => {
        const vault = _morphoVaults[i];
        const underlying = morphoVaults[vault];
        const decimals = vaultDecimals[i];
        const amount = underlying === USDT0 ? adjustDecimals(data, decimals) : data;
