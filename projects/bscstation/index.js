...
async function tvl(api) {
  const owners = startPools.concat(poolTokens)
  const tokens = await api.multiCall({  abi: "address:stakedToken", calls: owners})
  return sumTokens2({ tokensAndOwners2: [tokens, owners], api, resolveLP: true, })
}

module.exports = {
  bsc: {
    tvl,
    staking: staking(stakingPools, token)
  },
}