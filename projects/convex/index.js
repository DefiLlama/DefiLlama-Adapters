...
const btcReplacements = ["0x075b1bb99792c9E1041bA13afEf80C91a1e70fB3"];
...
async function tvl(timestamp, block) {
  ...
  await Promise.all(
    [...Array(Number(poolLength)).keys()].map(async (i) => {
      ...
      var share = BigNumber(convexsupply.output)
        .times(1e18)
      ...
      if (pool.output == addressZero) {
        ...
        if (maincoins.output.length == 0) {
          ...
          console.log(
            "pool " + i + " is a factory pool, use lptoken as swap address"
          );
      ...
      if (i == 29) {
        ...
        coins = coins.map((result, i) => ({
          ...
          balance: BigNumber(result.balance)
      ...
      for (var c = 0; c < coins.length; c++) {
        var balanceShare = BigNumber(coins[c].balance.toString())
          .times(share)
          .div(1e18)
          .toFixed(0);
        ...
        if (btcReplacements.includes(coinAddress)) {
          ...
          balanceShare = BigNumber(balanceShare.toString())
            .div(1e10)
            .toFixed(0);
        }
