...
async function getChainBalances(allTokens, chain, block, transform = (a) => a) {
  ...
  decimals.output.forEach((call) => {
    const tokens = allTokens.filter(
      (token) => token.underlyingAddress === call.input.target
    );
    let decimals = call.output;
    tokens.forEach((token) => {
      // Some abi calls error out, replace known decimals count
      if (
        token.underlyingAddress === "0x2260fac5e5542a773aa44fbcfedf7c193bc2c599"
      )
        decimals = "8"; // WBTC
      else if (
  ...
  lockedTokensOutput.output.forEach((call) => {
    ...
    let underlyingTokenBalance = BigNumber(call.output)
      .div(10 ** (18 - decimals))
      .toFixed(0);
    ...
    if (tokensNativeToSidechain.includes(token.id))
      prefixedUnderlyingAddress = chain + ":" + token.id;
