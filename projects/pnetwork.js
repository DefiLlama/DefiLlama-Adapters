...
function getChainTvl(chain) {
  async function current() {
    ...
    return response.data
      .filter((c) => c.hostBlockchain === chain)
      .reduce((total, asset) => total + asset.tvl_number, 0);
  }

  async function historical() {
    let response = await axios.get(historicalUrl);
    return response.data.results[0].series
      .find((s) => s.tags.host_blockchain === chain)
      .values.map((d) => ({
        date: d[0] / 1000,
...
module.exports = {
  ...
  methodology:
    "Queries the pNetwork database, using the same API endpoint as their own UI. TVL is based on the amount of assets “locked” in the system and that therefore has a 1:1 tokenisation on a host blockchain, including all of the assets and all of the blockchains supported by pNetwork.",
};
