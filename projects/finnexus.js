...
const web3wan = new Web3(
  new Web3.providers.HttpProvider("https://gwan-ssl.wandevs.org:56891")
);
...
async function fetch() {
  ...
  try {
    let ethUsdcTotal = await getEthTotalCollateral(
      abis.abis.fnxOracle,
      ethUsdcPool
    );
    let ethFnxTotal = await getEthTotalCollateral(
      abis.abis.fnxOracle,
      ethFnxPool
    );
    let ethFraxTotal = await getEthTotalCollateral(
      abis.abis.fnxOracle,
      ethFraxPool
    );
    ...
    tlv =
      parseFloat(ethUsdcTotal) +
      parseFloat(ethFnxTotal) +
      parseFloat(ethFraxTotal);
  } catch (e) {
    console.log(e);
  }

  try {
    let wanFnxTotal = await getWanTotalCollateral(
      abis.abis.fnxOracle,
      wanFnxPool
    );
    let wanUsdcTotal = await getWanTotalCollateral(
      abis.abis.fnxOracle,
      wanUsdcPool
    );
  ...
  try {
    let bscFnxTotal = await getBscTotalCollateral(
      abis.abis.fnxOracle,
      bscFnxPool
    );
    let bscUsdcTotal = await getBscTotalCollateral(
      abis.abis.fnxOracle,
      bscUsdPool
    );
