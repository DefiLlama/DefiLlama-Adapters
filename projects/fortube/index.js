...
async function bsc(_timestamp, _ethBlock, chainBlocks) {
  ...
  const markets = await axios.get(
    "https://api.for.tube/api/v1/bank/public/chain/BSC-Inno/markets"
  );
  ...
  tokenBalances.output.map((call, idx) => {
    ...
    if (symbol.startsWith("v")) {
      if (symbol === "v4Belt" || symbol === "v3EPS") {
        ...
        unwrapCalls.push(
          unwrapVaultLP(balances, token, call.output, chainBlocks["bsc"])
        );
      }
    } else {
...
async function okexchain(_timestamp, _ethBlock, chainBlocks) {
  const markets = await axios.get(
    "https://api.for.tube/api/v1/bank/public/chain/OEC-Inno/markets"
  );
  const usdCollateral = markets.data.data.reduce(
    (acc, val) => acc + val.token_price * val.balance_in_cheque,
    0
  );
