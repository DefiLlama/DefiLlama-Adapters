...
async function fetch() {
  let price_feed = await retry(
    async (bail) =>
      await axios.get(
        "https://api.coingecko.com/api/v3/simple/price?ids=ripple,hard-protocol,binancecoin,kava,bitcoin&vs_currencies=usd&include_market_cap=true&include_24hr_vol=true&include_24hr_change=true"
      )
  );

  let cdps = await retry(
    async (bail) =>
      await axios.get("https://kava4.data.kava.io/cdp/cdps?limit=5000")
  );
