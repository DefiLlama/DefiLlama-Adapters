...
//V3
const routers = [
  "0xe3cF69b86F274a14B87946bf641f11Ac837f4492",
  "0xe6887c0cc3c37cb2ee34Bc58AB258f36825CA910",
  "0xE540998865aFEB054021dc849Cc6191b8E09dC08",
  "0xC6C68811E75EfD86d012587849F1A1D30427361d",
];
...
async function getRouterBalances(timestamp, chain, settings, block) {
  ...
  const totalNativeBalance = nativeBalances.output.reduce(
    (acc, output) => acc.plus(output.balance),
    BigNumber(0)
  );
  const balances = {};
  balances[settings.nativeCoin] = totalNativeBalance.div(1e18).toFixed(0);
  routerBalances.output.forEach((result) => {
    const tokenIndex = settings.tokens.findIndex(
      (token) =>
        result.input.target.toLowerCase() === token.address.toLowerCase()
    );
