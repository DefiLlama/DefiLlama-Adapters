...
const NAKED_STAKING = require("./config/mantra-dao/NAKED_STAKING.json");
...
const COMPOUND_LENS_ABI = require("./config/mantra-dao/COMPOUND_LENS_ABI.json");
const EXP_DECIMALS = 18;
const toBN = (weiStr) => new BigNumber(weiStr);
const toBNScaled = (value, decimal = 0) => toBN(value).div(10 ** decimal);
...
async function fetchETH() {
  ...
  const stakingAssetsETH = [
    ...
    // SKYRIM staking
    {
      contract: "0x93666797a2AE5cc8876D9Bc00A67ECDCc6e7A4Fa",
      token: "0x2610F0bFC21EF389fe4D03CFB7De9ac1E6C99D6E",
      price: "skyrim-finance",
    },
    // CIRUS staking
    {
      contract: "0x21da0e4595f15d3037BDe7BC17C5941C9958606D",
      token: "0xA01199c61841Fce3b3daFB83FeFC1899715c8756",
      price: "cirus",
    },
    // UMAD staking
    {
      contract: "0x561fE2F3323b92d5a1C50566f07Eb5e029E60A46",
      token: "0x31c2415c946928e9FD1Af83cdFA38d3eDBD4326f",
      price: "madworld",
    },
  ];
  ...
  async function returnBalanceETHLP(token, address) {
    ...
    balance = await new BigNumber(balance).div(10 ** decimals);
  ...
  async function getPriceOfUniPair(pair) {
    ...
    let uniPairContractService = new web3.eth.Contract(
      UNI_LP,
      pair.pairAddress
    );
    let decimals = await uniPairContractService.methods.decimals().call();
    const totalSupplyScaledDown =
      (await uniPairContractService.methods.totalSupply().call()) /
      10 ** decimals;
    const token1Supply = await uniPairContractService.methods
      .getReserves()
      .call();
    const token1SupplyScaledDown = token1Supply._reserve0 / 10 ** decimals;
    lpTokenPrice =
      ((token1SupplyScaledDown * token1price) / totalSupplyScaledDown) * 2;
    return lpTokenPrice;
  }
  ...
  await Promise.all(
    lpStakingAssetsETH.map(async (pair) => {
      let lpTokenPrice = await getPriceOfUniPair(pair);
      let balance = await returnBalanceETHLP(pair.pairAddress, pair.contract);
...
async function fetchBSC() {
  ...
  async function returnBalance(address) {
    let contract = new web3bsc.eth.Contract(NAKED_STAKING, address);
    let decimals = await contract.methods.decimals().call();
    let balance = await contract.methods.totalStaked().call();
    balance = await new BigNumber(balance).div(10 ** decimals);
    return parseFloat(balance);
  }

  async function returnBalanceBSCLP(token, address) {
  ...
  async function getPriceOfPancakePair(pair) {
    let lpTokenPrice = 0;
    const token1price = price_feed.data[pair.price1].usd;
    let pancakePairContractService = new web3bsc.eth.Contract(
      PANCAKE_LP,
      pair.pairAddress
    );
    let decimals = await pancakePairContractService.methods.decimals().call();
    const totalSupplyScaledDown =
      (await pancakePairContractService.methods.totalSupply().call()) /
      10 ** decimals;
    const token1Supply = await pancakePairContractService.methods
      .getReserves()
      .call();
    const token1SupplyScaledDown = token1Supply._reserve0 / 10 ** decimals;
    lpTokenPrice =
  ...
  // BSC NAKED STAKING ASSETS
  await Promise.all(
    stakingAssetsBSC.map(async (asset) => {
      let balance = await returnBalance(asset.contract);
  ...
  await Promise.all(
    lpStakingAssetsBSC.map(async (pair) => {
      let lpTokenPrice = await getPriceOfPancakePair(pair);
      let balance = await returnBalanceBSCLP(pair.pairAddress, pair.contract);
...
async function fetchPolygon() {
  ...
  async function returnBalance(address) {
    let contract = new web3polygon.eth.Contract(NAKED_STAKING, address);
    let decimals = await contract.methods.decimals().call();
    let balance = await contract.methods.totalStaked().call();
    balance = await new BigNumber(balance).div(10 ** decimals);
    return parseFloat(balance);
  ...
  async function getPriceOfQuickPair(pair) {
    let lpTokenPrice = 0;
    const token1price = price_feed.data[pair.price1].usd;
    let quickPairContractService = new web3polygon.eth.Contract(
      QUICK_LP,
      pair.pairAddress
    );
    let decimals = await quickPairContractService.methods.decimals().call();
    const totalSupplyScaledDown =
      (await quickPairContractService.methods.totalSupply().call()) /
      10 ** decimals;
    const token1Supply = await quickPairContractService.methods
      .getReserves()
      .call();
    const token1SupplyScaledDown = token1Supply._reserve0 / 10 ** decimals;
    lpTokenPrice =
  ...
  await Promise.all(
    lpStakingAssetsPOLYGON.map(async (pair) => {
