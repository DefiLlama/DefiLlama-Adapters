...
async function fetch() {
  try {
    ...
    const zenErc20 = [
      ...
      // zenCOMP -
      [
        "0x3f2e9a93428a22d2f4cacc3f184f1aad85054e1c",
    ...
    await Promise.all(
      lpStakingAssets.map(async (pair) => {
        // LP STAKING ASSETS
        try {
          let lpTokenPrice = await getPriceOfUniPair(pair);
          let balance = await utils.returnBalance(
            pair.pairAddress,
            pair.contract
          );
