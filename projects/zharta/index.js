...
function fetchEstimatedValue() {
  const response = axios.get("https://api.zharta.io/lending_pools/v2").then(res => res.data.lending_pools[0].collateral_estimated_value);
  return response;
}
...
module.exports = {
  ...
  ethereum: {
    tvl: () => {
      const collateralEstimatedValue = fetchEstimatedValue();
      const tokensTvl = sumTokensExport({
        ownerTokens: [
      ...
      return collateralEstimatedValue.then(value => Number(value)) + tokensTvl;
    },
  },
};
