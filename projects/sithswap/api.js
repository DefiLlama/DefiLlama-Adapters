...
async function tvl() {
  ...
  const [ token0s, token1s ,reserves0,reserves1] = await Promise.all([
    multiCall({ abi: abi.pair.getToken0, calls }),
    multiCall({ abi: abi.pair.getToken1, calls }),
    multiCall({ abi: abi.pair.getReserve0, calls }),
    multiCall({ abi: abi.pair.getReserve1, calls }),
  ])
