...
async function calculateTvl(contract, chain, block) {
    ...
    let tokensAndOwners = await mapTokensToContract(
        contract,
        chain,
        block,
        lengths
    ...
    for (const key in balancesB) {
        ...
        if (balancesC.hasOwnProperty(key)) {
            balancesC[key] = (
                BigInt(balancesC[key]) + BigInt(balancesB[key])
            ).toString()
        } else {
...
async function getBalances(contract, chain, block, length) {
    const calls = []
    for (let i = 1; i <= length; i++) calls.push({target: contract, params: i})
    ...
    return await vestingHelper({
        ...
        block,
        chain,
        log_coreAssetPrices: [300 / 1e18, 1 / 1e18, 1 / 1e18, 1 / 1e18],
...
const chains = ['arbitrum', 'bsc', 'polygon', 'core', 'dogechain', 'metis']
...
chains.forEach((chain) => {
    ...
    switch (chain) {
        case 'arbitrum':
            contract = config.kimberliteSafeARB.locker
            break
        case 'metis':
            contract = config.kimberliteSafeMETIS.locker
            break
        case 'polygon':
            contract = config.kimberliteSafeMATIC.locker
            break
        case 'core':
            contract = config.kimberliteSafeCORE.locker
            break
        case 'dogechain':
            contract = config.kimberliteSafeDOGE.locker
            break
    }
    ...
    module.exports[chain] = {
        ...
        tvl: async (_, _b, {[chain]: block}) =>
            calculateTvl(contract, chain, block),
    }
})
