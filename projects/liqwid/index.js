...
async function tvl() {
  ...
  const utxoValues = (await Promise.all(scriptAddresses.map(getAssets))).flat();
  const assetKeyMapping = {
    "8db269c3ec630e06ae29f74bc39edd1f87c819f1056206e879a1cd61446a65644d6963726f555344": "djed",
    "lovelace": "cardano"
  };
  const assetTotals = Object.fromEntries(Object.entries(assetKeyMapping).map(([valueName, assetName]) => {
    const total =
      (utxoValues
        .filter(entry => entry.unit === valueName)
        .map(entry => parseInt(entry.quantity) / 1e6)
