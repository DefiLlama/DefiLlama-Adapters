    ...
    for (const [idx, data] of Object.entries(rate_limits.data)) {
        ...
        try {
            ...
            try {
                ...
                console.error(`Error deserializing value for token ${data.tokenAddress}:`, deserializeError.message);
            }

        } catch (error) {
            console.error(`Error fetching MASP balance for token ${data.tokenAddress}:`, error.message);
        }
    }
}
