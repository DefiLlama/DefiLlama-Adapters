...
const providerUrl =
  "https://eth-mainnet.alchemyapi.io/v2/NR1gYkSQEB7lu0q-cONMsxRLwFct0luH";
...
const getBTokenTotalSupply = (bTokenSymbol) => (precision) =>
  getContractInstance(provider)(constants.getBTokenAddress(bTokenSymbol))(
    abis.bVault
  )
    .totalSupply()
    .then((totalSupply) =>
      extractBigNumber(totalSupply.toString())(
        constants.getBTokenDecimal(bTokenSymbol)
      )(precision)
    );

const getErc20TokenBalance =
  (erc20TokenSymbol) => (uniSwapLpTokenSymbol) => (precision) =>
    getContractInstance(provider)(constants.getTokenAddress(erc20TokenSymbol))(
      abis.erc20
    )
      .balanceOf(constants.getUniSwapLpTokenAddress(uniSwapLpTokenSymbol))
      .then((balance) =>
        extractBigNumber(balance.toString())(
          constants.getTokenDecimal(erc20TokenSymbol)
        )(precision)
      );

const getBTokenPricePerFullShare =
  (bTokenSymbol) => (decimals) => (precision) =>
    getContractInstance(provider)(constants.getBTokenAddress(bTokenSymbol))(
      abis.bVault
    )
...
const getBVaultTvl =
  (baseTokenPriceInUsd) => (bTokenSymbol) => (decimals) => (precision) => {
    const bTokenPricePerFullShare =
      getBTokenPricePerFullShare(bTokenSymbol)(decimals)(precision);
    const bTokenTotalSupply = getBTokenTotalSupply(bTokenSymbol)(precision);
    return calculateTvl(baseTokenPriceInUsd)(bTokenPricePerFullShare)(
      bTokenTotalSupply
    );
  };
...
const sumTvls = (bVaultSymbols) => (coinGeckoIdMap) => (precision) =>
  bVaultSymbols
    .map((symbol) => {
      const baseTokenPriceInUsd = getTokenPriceCoinGecko("usd")(
        coinGeckoIdMap[symbol]
      );
      const tvl = getBVaultTvl(baseTokenPriceInUsd)(symbol)(18)(precision);
      tvl.then((tvl) => console.log(symbol + " TVL: " + tvl));
      return tvl;
    })
    .reduce((accumulatedTvl, nextTvl) =>
      Promise.all([accumulatedTvl, nextTvl]).then(
        ([accumulatedTvl, nextTvl]) => accumulatedTvl + nextTvl
      )
    );

const sumLiquidityMiningTvls =
  (liquidityMiningSymbols) => (coinGeckoIdMap) => (precision) =>
...
const fetch = async () => {
  ...
  const lmTvl = sumLiquidityMiningTvls(liquidityMiningSymbols)(coinGeckoIdMap)(
    10
  );
