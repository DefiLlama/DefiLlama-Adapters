...
async function tvl(timestamp, block, chainBlocks) {
  ...
  async function sumBalance(token, address) {
  ...
  // Calculate the value of assets in the treasury
  await sumBalance(BUSD_TOKEN_CONTRACT, TREASURY_ADDRESS);
  await sumBalance(USDT_TOKEN_CONTRACT, TREASURY_ADDRESS);
  await sumBalance(USDC_TOKEN_CONTRACT, TREASURY_ADDRESS);
  await sumBalance(WBNB_TOKEN_CONTRACT, TREASURY_ADDRESS);

  // Calculate the value of locked assets in P2P trade escrow contract
  await sumBalance(BUSD_TOKEN_CONTRACT, DOTC_CONTRACT);
  await sumBalance(USDT_TOKEN_CONTRACT, DOTC_CONTRACT);
  await sumBalance(USDC_TOKEN_CONTRACT, DOTC_CONTRACT);
  await sumBalance(BTCB_TOKEN_CONTRACT, DOTC_CONTRACT);
  await sumBalance(USTC_TOKEN_CONTRACT, DOTC_CONTRACT);
